...
| Syntax      | Single delimiter `[]`, meaning by position: value computation (head + args), `[do …]` sequence/block, type, binding list (params/struct fields/destructure), body. A line not starting with `[` is a command application. `{}` is a legacy alias (lowers to the same AST), being retired — see `BRACKET_FLIP_DESIGN.md`. Bare words are strings; `$var` reads a variable. Binding operators (`=`, `:`, `::`) sugar `def`/`mut`/`set`. Full reference: `SYNTAX.md`.                |
