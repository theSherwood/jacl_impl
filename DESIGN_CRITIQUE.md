...
| ~~`sleep N` (suspending)~~ | — | shipped 2026-05-18 (~250 LOC actual) | `OP_SLEEP_SM` suspends via a runtime-owned timer thread (min-heap of deadlines, SM pinned as external GC root); `OP_SLEEP_BLOCK` nanosleeps in non-SM contexts. Side-effect: moved the compile-time suspension-analysis pass to run *after* macro expansion, so macros expanding into suspending forms (timeout) get correctly recognized. |
| ~~`timeout N {body}`~~ | — | shipped 2026-05-18 (~3 LOC macro) | Prelude macro `[timeout n body] → [race ~body { sleep ~n; error "timeout" }]`. Macro-expansion fast-path is now driven by an auto-generated list (`src/prelude_macro_names.h`, regen'd from `prelude.jacl` by `build.sh`), so any future `defmacro` in the prelude joins the fast-path automatically. The dead operator macros (`=`, `:`, `|`, `::`) were deleted from the prelude in the same pass — they would have miscompiled typed bindings if they ever fired (parser-level forms collide). |
