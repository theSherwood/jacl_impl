...
the current `temen` API). As of the item-6 migration it runs on the emit-only driver:
**96 pass, 2 `#[ignore]`d** (see the driver note below).

**What was fixed.** The harness used the pre-`IMPORTS.md`-phase-4 link/run model
(`temen_ir::link` + `synth_powerbox_start(linked, entry, 3, false)` + `instantiate`), which no
...
(`bin/jacl_temen.rs`): `link_with_manifest` → export + `resolve_export` the program entry →
...
now exactly the TEMEN backend's own behavior (typer + codegen). This is a prerequisite for
deleting `compiler.c`/`vm.c` (slice 3c): the oracle path links against them. Effect on the
previously-ignored cases: `arithmetic_variadic_fold` is **un-ignored** (TEMEN codegen folds `+`
...
- `binding_same_scope_shadow_is_an_error` — TEMEN codegen deliberately permits top-level
...
by `codegen/selfhost/macro_staging/run_diff.sh` (legacy oracle) and `--temen` (staged), and by the
`jacl_temen` binary on the `test/jacl` corpus.
...
1. *temen-text canonical drift.* `irb_to_text` emitted `func (…)` with unindented blocks, but the
   vendored `temen_text::print_module` now emits `func <idx> (…)` with 2-space block / 4-space body
...
2. *link-model drift.* The 3 tests that link against the translated runtime (`closure`, `gc`,
   ...
   a `write` **manifest capability import** that the old `temen_ir::link` + direct `interp/jit` path
   ...
   `run_diff`. Marked `#[ignore]` with that pointer; this is runtime-on-TEMEN work (staging items
...
uses the retired `block0(…):` label syntax, and the old `temen_ir::link` + direct interp/jit can't
