/* codegen.h — JACL AST → TEMEN-IR code generation (P2.2 scaffold).
 *
 * The Phase-2 codegen walk: consumes the JACL AST (post-parse, optionally typed)
 * and emits an TEMEN-IR module via the IR builder (irbuilder.h). The scaffold lowers
...
/* Compile an array of top-level AST nodes into an TEMEN-IR module with a single entry
...
/* `in_guest` selects the entry ABI (docs/TEMEN_GUEST_JIT_STAGING.md §5):
...
IrModule *temen_codegen_program(struct AstNode **nodes, uint32_t count, int module_mode, int in_guest,
...
int temen_program_uses_concurrency(struct AstNode **nodes, uint32_t count);

/* Compile one macro body to a module with a single `__jacl_macro(sp, args…) -> syntax-vec`
 * function — the codegen half of staged macro evaluation (docs/TEMEN_MACRO_STAGING_PLAN.md,
 ...
 * construct (message in `err`). See docs/TEMEN_MACRO_STAGING_PLAN.md "final link".
 *
 * `in_guest` selects the entry (func 0) ABI. 0 (native / powerbox): arity-1 `(sp) -> i64`, which
 * `synth_manifest_start` wraps in `_start`. 1 (guest-JIT, docs/TEMEN_GUEST_JIT_STAGING.md §3d):
