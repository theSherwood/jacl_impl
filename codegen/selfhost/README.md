...
Build the JACL **compiler itself** to an SVM module (`jacl_compiler.temen`) and run it
...
| `build_compiler_svmb.sh` | Compile the frontend + driver + shim to LLVM IR → `llvm-link` → `svm-llvm-translate` → `build/jacl_compiler.temen`. `--selftest` also compiles a fixed program on the SVM engine and checks the emitted IR. |
...
`jacl_compiler.temen` reads source on stdin, writes SVM IR on stdout. In the browser it
runs on `svm_browser.wasm` via `svm_run_onramp` (stdin = the edited source); the
emitted IR then goes to the generic `svm_link_run` (link vs `jaclrt.temen` + run) — the
