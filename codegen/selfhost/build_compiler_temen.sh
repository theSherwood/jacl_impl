#!/usr/bin/env bash
#
# Build the JACL compiler as an TEMEN guest: compile the C frontend to LLVM IR, run it
# through the vendor/temen LLVM on-ramp, and emit `jacl_compiler.temen` — a module that
# reads JACL source on stdin and writes TEMEN-IR text on stdout.
#
#   codegen/selfhost/build_compiler_temen.sh [--selftest]
#
# Env overrides: CLANG (clang-18), LLVM_LINK (llvm-link-18), TEMEN_LLVM_TRANSLATE and
# TRY_TRANSLATE (prebuilt on-ramp binaries; otherwise built from vendor/temen).
#
# Requires clang-18 + llvm-link-18. Skips cleanly (exit 0) if they are absent, like
# the temen demo builds, so it can sit in a build that lacks the dev toolchain.
# Rationale + measured feasibility: docs/TEMEN_SELFHOST_FEASIBILITY.md.
set -euo pipefail
...
if ! command -v "$CLANG" >/dev/null || ! command -v "$LLVM_LINK" >/dev/null; then
  echo "note: $CLANG / $LLVM_LINK not found — skipping JACL-on-TEMEN build (dev toolchain absent)."
  exit 0
fi

# On-ramp binaries: use prebuilt if given, else build from the vendored temen submodule.
TRANSLATE="${TEMEN_LLVM_TRANSLATE:-$TEMEN/crates/temen-llvm/target/release/temen-llvm-translate}"
TRY="${TRY_TRANSLATE:-$TEMEN/crates/temen-llvm/target/release/examples/try_translate}"
if [ ! -x "$TRANSLATE" ]; then
  echo "=== building temen-llvm-translate from vendor/temen ==="
  ( cd "$TEMEN/crates/temen-llvm" && cargo build --release --bin temen-llvm-translate --example try_translate )
fi

# -DJACL_EMIT_ONLY selects the emit-only frontend unity (src/jacl_emit.c) inside emit_jacl.c:
# the compiler-as-guest only compiles JACL -> TEMEN IR, so the legacy bytecode backend
# (bytecode.c/compiler.c/vm.c) + its scheduler/embedding (runtime.c/embed.c) are dropped —
# a smaller .temen, no stubbed vm.c externals (docs/TEMEN_SELFHOST_FEASIBILITY.md §2, item 6).
# The guest uses the engine's synthesized `vm_map`-growing allocator (emit_shim.c no longer defines a
# fixed arena — retired once temen #1312/#1319 made the coop window growable; see TEMEN_WARM_COMPILER.md).
...
# In-guest macro staging (docs/TEMEN_GUEST_JIT_STAGING.md §3/§4): the jaclrt runtime + staged-macro
# glue (jaclrt_staging_guest.c) and the Jit-capability hook (stage_bridge_guest.c), linked in so
# macros expand on the TEMEN engine in this same domain instead of the reference VM. Needs `-I runtime`.
...
# In-guest `[interpret …]` (docs/TEMEN_GUEST_JIT_STAGING.md §5, model B): compile+run the source on the
...
echo "=== translating to TEMEN IR (jacl_compiler.temen) ==="
...
# Warm-snapshot two-phase card (docs/TEMEN_WARM_COMPILER.md Slice 2/3): the same program, but driven
...
if [ "${1:-}" = "--selftest" ]; then
  echo "=== selftest: compile a fixed JACL program on the TEMEN engine ==="
...
EOF
  ...
  if grep -q 'i32.mul' "$OUT/selftest.out" && grep -q 'i32.add' "$OUT/selftest.out"; then
    echo "SELFTEST PASS — JACL source compiled to TEMEN IR on the TEMEN engine."
  else
  ...
  # Each corpus program runs the whole compile — including its macro expansion — on the TEMEN engine,
...
EOF
  ...
  # `[interpret SRC]` with the in-guest interpret hook installed: SRC is compiled (temen_codegen_program
...
int main(void) {
  ...
  jacl_install_temen_stage_hook();       /* so macros in interpreted source expand in-guest */
  jacl_install_temen_interpret_hook();
  run("PLAIN", "[+ 1 [* 2 3]]", 13);
...
EOF
  ...
  # (docs/TEMEN_WARM_COMPILER.md) before the browser reactor wiring (Slice 3).
