 ...
 * single __jacl_macro(sp, params…) function (temen_codegen_macro_body), print the temen-text.
...
int main(int argc, char **argv) {
   ...
   * on-TEMEN run); default emits just the __jacl_macro function (for inspection / linking). */
  ...
  IrModule *m;
  if (staged) {
    m = temen_codegen_staged_macro((const char **)dm->data.defmacro.param_names,
    ...
    uint32_t idx = 0;
    m = temen_codegen_macro_body((const char **)dm->data.defmacro.param_names,
                               dm->data.defmacro.param_name_lens,
                               dm->data.defmacro.param_count,
                               dm->data.defmacro.body, &idx, err, sizeof err);
  }
  if (!m) { fprintf(stderr, "codegen: %s\n", err); return 1; }
  /* Emit the temen-encode **object** (v9); stage_macro.rs decodes it with `decode_emitted`.
