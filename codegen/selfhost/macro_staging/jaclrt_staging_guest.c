 ...
 * buffer-based staged-macro glue, linked INTO the compiler `.temen` (build_compiler_temen.sh) so a
 ...
 * {name -> Slot} symtab from `&fn`). Compile with `-I runtime`. See docs/TEMEN_GUEST_JIT_STAGING.md
 ...
 *
 * **Symbol separation.** The compiler `.temen` already embeds the *reference VM's* runtime
 ...
 * collision check in build_compiler_temen.sh (it fails the build if the set drifts). */
