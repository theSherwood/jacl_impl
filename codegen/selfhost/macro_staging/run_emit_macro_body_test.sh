...
# output parses as temen-text. This tool feeds the codegen'd __jacl_macro into the staged-
# macro link (see docs/TEMEN_MACRO_STAGING_PLAN.md "final link"). Needs gcc; skips if absent.
set -euo pipefail
...
if printf '%s' "$out" | grep -qE '^func \(' && printf '%s' "$out" | grep -q 'jacl_vec_empty'; then
  echo "  PASS  twice -> __jacl_macro temen-text (builds plain-data vec)"
else
