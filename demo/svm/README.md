...
| File | Role |
|---|---|
| `build_assets.sh` | Build `svm_browser.wasm` (cdylib → wasm32) + `jacl_emit.wasm` (frontend) into `demo/wasm/`, ship `jaclrt.temen`, and link+encode example programs to `svm/svmb/*.temen` (+ `manifest.json`). |
...
| `run_svmb.mjs` | Headless Node driver — runs a `.temen` through `svm_run_onramp` and prints captured stdout. CI gate for the precompiled run path. |
...
cd demo && bash svm/build_assets.sh          # → demo/wasm/svm_browser.wasm + demo/svm/svmb/*.temen
node svm/run_svmb.mjs wasm/svm_browser.wasm svm/svmb/hi.temen  $'hi\n'    # PASS
node svm/run_svmb.mjs wasm/svm_browser.wasm svm/svmb/gen.temen $'1\n2\n'  # PASS
```
...
A `.temen` off `emit_svmb` (frontend → link vs `jaclrt.temen` → `synth_manifest_start` →
...
2. **Edited source (live):** `JaclFrontend.emitIr(source)` compiles it to self-contained SVM
   ...
   `SvmJaclRunner.linkRun(ir, jaclrt.temen)` links it against the runtime and runs it. `linkRun`
