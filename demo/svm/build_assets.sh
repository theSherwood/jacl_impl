...
#
# The runtime IR (jaclrt.temen) is baked by runtime/build.sh (clang-18 + svm-llvm) — libLLVM
...
cp "$ROOT/runtime/build/jaclrt.temen" "$DEMO/svm/jaclrt.temen"
# Transition alias — REMOVE once .github/workflows/pages.yml is synced from workflows_src/: the live
# Pages workflow (this token can't edit it) still copies `jaclrt.svm` by name, so ship the same binary
# under the old name too. `temen_link_run` sniffs the format by magic, so the extension is cosmetic.
cp "$DEMO/svm/jaclrt.temen" "$DEMO/svm/jaclrt.svm"
echo "  -> demo/svm/jaclrt.temen ($(wc -c < "$DEMO/svm/jaclrt.temen") bytes; the live-editing link target)"
...
if [ -f "$ROOT/runtime/build/jaclrt_staging.temeno" ]; then
  mkdir -p "$DIR/svmb"
  cp "$ROOT/runtime/build/jaclrt_staging.temeno" "$DIR/svmb/jaclrt_staging.temeno"
  echo "  -> demo/svm/svmb/jaclrt_staging.temeno ($(wc -c < "$DIR/svmb/jaclrt_staging.temeno") bytes; jacl_emit macro staging)"
fi
...
# --- 1d. self-hosted frontend (jacl_compiler.temen): the JACL compiler translated to SVM, run as a
...
if [ -f "$ROOT/codegen/selfhost/build/jacl_compiler.temen" ]; then
  ...
  echo "  -> demo/svm/svmb/jacl_compiler.temen ($(wc -c < "$OUT/jacl_compiler.temen") bytes; the macro-capable live frontend)"
  ...
  if [ -f "$ROOT/codegen/selfhost/build/jacl_compiler_snapshot.temen" ]; then
    ...
    echo "  -> demo/svm/svmb/jacl_compiler_snapshot.temen ($(wc -c < "$OUT/jacl_compiler_snapshot.temen") bytes; warm-snapshot fast compile)"
  fi
else
...
for prog in "$@"; do
  ...
  ( cd "$ROOT/runtime/harness" && cargo run --quiet --bin emit_svmb -- "$prog" "$OUT/$name.temen" )
  echo "  -> demo/svm/svmb/$name.temen ($(wc -c < "$OUT/$name.temen") bytes)"
  names+=("$name")
done

# Pre-bake the macro-bearing tour so the *unedited* tour example loads instantly (runs its shipped
# .temen, no ~1.4s live compile). emit_svmb's C driver is emit-only and can't stage `defmacro`s, so the
...
if [ -f "$OUT/jacl_compiler.temen" ] && [ -f "$DEMO/wasm/temen_browser.wasm" ] && [ -f "$ROOT/test/jacl/tour.jacl" ]; then
  ...
  ( cd "$ROOT/runtime/harness" && cargo run --quiet --bin emit_svmb -- --ir "$OUT/tour.ir" "$OUT/tour.temen" )
  rm -f "$OUT/tour.ir"
  echo "  -> demo/svm/svmb/tour.temen ($(wc -c < "$OUT/tour.temen") bytes; the unedited-tour fast path)"
  names+=("tour")
fi
...
printf '%s\n' "${names[@]}" | node -e '
  ...
  const manifest = names.map((n) => ({ name: n, temen: `svmb/${n}.temen` }));
...
if [ -f "$OUT/jacl_compiler.temen" ]; then
  ...
  node "$DIR/concurrent_run.mjs" "$DEMO/wasm/temen_browser.wasm" "$OUT/jacl_compiler.temen" "$DEMO/svm/jaclrt.temen"
fi
...
if [ -f "$DEMO/wasm/jacl_emit.js" ] && [ -f "$DIR/svmb/jaclrt_staging.temeno" ]; then
  echo "Gate: macro-staging path (stage_gate.mjs)…"
  node "$DIR/stage_gate.mjs" "$DEMO/wasm/jacl_emit.js" "$DEMO/wasm/temen_browser.wasm" "$DIR/svmb/jaclrt_staging.temeno" "$DEMO/svm/jaclrt.temen"
fi

echo ""
echo "Smoke-test the run path headless:"
echo "  node demo/svm/run_svmb.mjs demo/wasm/temen_browser.wasm demo/svm/svmb/hi.temen \$'hi\\n'"
