...
// svm-browser cdylib's `svm_run_onramp` (wasm32) with a macro-bearing JACL program on stdin, and
...
//
//   node compile_guest.mjs <svm_browser.wasm> <jacl_compiler.svmb>
//
// Exit 0 iff the compile succeeds (svm_status == 0/exit) and the emitted IR contains a `br_if`
...
if (!wasmPath || !svmbPath) {
  console.error('usage: node compile_guest.mjs <svm_browser.wasm> <jacl_compiler.svmb>');
  process.exit(2);
}
...
const ir = capture(ex.svm_stdout_ptr, ex.svm_stdout_len);
