// ROI probe for the warm-snapshot / JIT compiler-guest plan (docs/TEMEN_WARM_COMPILER.md, Slice 0).
// Times the self-hosted compiler-guest (jacl_compiler.temen) compiling several programs through the
// temen-browser cdylib's temen_run_onramp — the exact playground live-compile path, on the bytecode
...
const cardPtr = load(card);
const cardLen = U(card.length);

function compileOnce(srcBytes) {
  const sp = load(srcBytes);
  ex.temen_run_onramp(cardPtr, cardLen, sp, U(srcBytes.length));
