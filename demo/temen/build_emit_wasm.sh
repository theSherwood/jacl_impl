...
# exposing `jacl_emit_ir(source) -> TEMEN IR text` (docs/TEMEN_BROWSER_PLAN.md, option (b), the live-
# editing half of the TEMEN playground). The JS host feeds edited source in, gets IR text back, then
# links it vs jaclrt.temen + encodes it in the browser and runs it through the temen-browser cdylib.
#
...
echo "=== Building jacl_emit.wasm (frontend → TEMEN IR) ==="
$EMCC \
  -std=c99 \
  -O2 \
  -D_DEFAULT_SOURCE \
  ...
  -s EXPORTED_FUNCTIONS="['_jacl_emit_ir','_jacl_install_temen_stage_hook','_malloc','_free']" \
