// Headless gate for the **self-hosted compiler path**: run `jacl_compiler.temen` through the
// temen-browser cdylib's `temen_run_onramp` (wasm32) with a macro-bearing JACL program on stdin, and
// assert the emitted TEMEN-IR shows the macro expanded. This exercises the real deployed path the
// playground's live-edit uses (`TemenJaclRunner.emitIrViaCompiler`) and gates two things the wasm32
...
//
//   node compile_guest.mjs <temen_browser.wasm> <jacl_compiler.temen>
//
...
const [wasmPath, cardPath] = process.argv.slice(2);
if (!wasmPath || !cardPath) {
  console.error('usage: node compile_guest.mjs <temen_browser.wasm> <jacl_compiler.temen>');
  process.exit(2);
}
