...
// build with no TEMEN runtime, so on its own it *cannot* expand a `defmacro` — it returns an
...
// frontend codegens each macro body to an TEMEN object and bounces it to the cdylib
// (jacl_temen_stage → Module.temenStageRun → temen_link_run vs jaclrt_staging.temeno), so only tiny macro
// bodies touch TEMEN and the tour compiles in ~80ms instead of ~1.8s.
//
...
// this by staging the macro body on TEMEN; without the hook it errors "emit-only build".
const SRC =
...
// --- temen-browser cdylib: runs the staged macro-body module, and the final linked program. ---------
...
// linkRunRaw twin (demo/src/temen-jacl-wasm.ts): link `moduleBytes` at __jacl_entry against `runtime`,
...
mod.temenStageRun = (moduleBytes, argWire) => { stageCalls++; return linkRunRaw(moduleBytes, stagingRt, argWire); };
if (typeof mod._jacl_install_temen_stage_hook !== 'function') {
  console.error('stage_gate: FAIL — jacl_emit.wasm has no jacl_install_temen_stage_hook export; the staging seam was not built in.');
  process.exit(1);
}
mod.ccall('jacl_install_temen_stage_hook', null, [], []); // arm the hook
...
if (stageCalls === 0) {
  console.error('stage_gate: FAIL — jacl_emit produced IR but the TEMEN stage hook never ran; the `unless` macro was not staged.');
  process.exit(1);
}
