// Emscripten JS-library: provides `jacl_temen_stage` to jacl_emit.wasm so the AOT-compiled frontend can
// stage macros by bouncing just the macro-body module to the TEMEN engine (the temen-browser cdylib).
...
//
// The actual TEMEN run is provided by the host via `Module.temenStageRun(moduleBytes, argWire) ->
// Uint8Array | null` — wired up in the playground to link the body against jaclrt_staging.temen and run
// it on the cdylib. Everything crossing this boundary is raw bytes; there is no AST/memory marshalling.
addToLibrary({
  jacl_temen_stage__deps: ['malloc', 'free'],
  jacl_temen_stage: function (modPtr, modLen, argPtr, argLen, outPtrPtr, outLenPtr) {
    ...
    let res = null;
    try {
      res = typeof Module.temenStageRun === 'function' ? Module.temenStageRun(mod, arg) : null;
    } catch (e) {
      res = null;
    }
  ...
  jacl_temen_stage_free__deps: ['free'],
  jacl_temen_stage_free: function (ptr, _len) {
    _free(ptr);
  },
});
