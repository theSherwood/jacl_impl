# JACL on TEMEN — Backend Rewrite Design

> Status: **design, pre-implementation.** Tracks the architecture for retargeting
> JACL's backend from its bytecode VM to **TEMEN** (`vendor/temen`, register-SSA
> sandbox VM with a Cranelift JIT). Companion docs: `TEMEN_BACKEND_PLAN.md` (phased
> plan, risks, spikes) and `TEMEN_GC_CONTRACT.md` (the GC interface, already landed
> on the temen side).

## 1. Why

TEMEN is our own VM (`github.com/theSherwood/vm`), the platform-independent layer
beneath a future plan9-like OS. Building JACL on it:

- **One VM to maintain, not two.** JACL's bytecode VM + epoch GC + state-machine
  concurrency are replaced by TEMEN's interpreter/JIT + fibers; we delete most of
  JACL's hardest runtime.
- **JACL as a concrete consumer** that hardens TEMEN (it already drove the
  `gc.roots` + `ref` work).
- **JACL as the eventual shell** for the TEMEN/plan9 layer — gradual typing is a
...
- Targeted today via **text IR** (chibicc C frontend proves it) and a binary
  encoding; the IR-building API is the Rust `temen-ir` crate. (See Plan §Spike-1 —
...
`value.c` (~0.9k). JaclVal is an 8-bit tag + 56-bit payload. TEMEN pointers are
...
every emit site changes from **bytecode → TEMEN IR**, plus it must:
...
`runtime.c`'s NxM Chase-Lev work-stealing design ports onto TEMEN fibers +
`thread.spawn` + futex. Structure reusable; primitives change.

### Deleted (the wins)
- **Bytecode VM dispatch + `bytecode.c`** (the dispatch loop; TEMEN executes now).
...
### 4.2 GC — conservative, non-moving, STW (see `TEMEN_GC_CONTRACT.md`)
- **Conservative roots** (JACL exposes raw C pointers, so values aren't
  tag-filterable). Root-finding is an temen service: `gc.roots(heap_lo, heap_hi, …)`
...
- M:N scheduler (Chase-Lev work-stealing) runs over TEMEN fibers + OS threads +
...
- **Deferred optimization, not now: a *narrow* stackless-generator lowering.**
  ...
  **flat** generator shape (yields at its own lexical level, no deep yield) into a
  ...
  suspended generators** (temen `durable`/`snapshot`) — a heap state object
...
- **JACL program codegen** (JACL source → IR): JACL's compiler emits TEMEN IR
  **directly** — its own in-memory IR-builder serialized to **text now**
  (debuggable; runs through temen's verifier + interp + the interp↔JIT differential
  oracle) and **binary (`temen-encode`) later** for compile speed, behind the same
...
- **JACL runtime library** (GC, allocator, scheduler, value ops, collections,
  ...
  LLVM bitcode → `temen-llvm` → TEMEN IR** (see §4.5). Built once as a prebuilt IR
  **artifact**.

**Codegen obligation — block-local SSA.** temen's IR is block-local: a block may
...
the real verifier/interp/JIT in `spikes/temen_emit_link` (Spike-1).

**Linking:** temen has static linking (`temen_ir::link`, `DYNLINK.md`) — merge the
...
the *spec* (and a cross-check oracle) for the intrinsic surface `temen-llvm` must
expose (§4.5).

### 4.5 Runtime via the LLVM on-ramp (reuse existing C)

The runtime runs as guest code, written in **C and compiled through `temen-llvm`**
(`crates/temen-llvm`) — temen's production LLVM-bitcode→IR translator. It is mature:
...
- **Reuse JACL's existing runtime C** — value-tag ops, the `lib/` collections
  ...
  builtin semantics are portable C that clang→`temen-llvm` compiles. The largest
...
JACL's window allocator over offsets (temen-llvm's malloc already grows the window;
ptr↔int works); the epoch GC → the new conservative collector; native threads →
temen fibers/threads. The *pure-computation* parts port nearly as-is.

**The temen-side gap is now closed** (temen `f75509a`, slices AC/AD/AF). `temen-llvm`
surfaces the full `<temen.h>` `__vm_*` set — 35 builtins incl. `__vm_gc_roots` →
...
GC, scheduler, and fiber generators are all expressible in C `#include <temen.h>` and
compiled through the on-ramp. (Originally the on-ramp lowered only ordinary C; the
ask in `TEMEN_LLVM_INTRINSICS_ASK.md` was implemented.)
...
### 4.6 Forward-compat
The `ref` `ValType` is reserved in TEMEN now (lowers as `i64`). If precise GC is
...
## 5. Non-goals
- No TEMEN-side GC or object model (mechanism-not-policy stays).
