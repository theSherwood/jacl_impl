# JACL on TEMEN — Phase 1 scope: the runtime substrate

> Status: **scoping, pre-implementation.** Branch: `temen-backend-phase1`. Companion
> to `TEMEN_BACKEND_DESIGN.md`, `TEMEN_BACKEND_PLAN.md` (§3 Phase 1), and
> `TEMEN_GC_CONTRACT.md`. Phase 0 (all four spikes) is **complete and merged** — the
...
`clang -O2 → temen-llvm → TEMEN IR`, with a test suite green on interp + JIT. When
Phase 1 is done, Phase 2 (JACL program codegen) has a substrate to emit calls
against.

**Definition of done:** `temen_ir::link`-ing a small program module against the
...
runtime/
  ...
  build.sh            # runtime C -> clang -O2 -emit-llvm -> temen-llvm -> artifact
  tests/              # C-driver "programs" linked against the runtime, run on interp+JIT
```

`<temen.h>` (from the submodule) supplies the `__vm_*` builtins (`__vm_map`,
...
- **Single-vCPU GC now.** Spike-2's model (conservative roots via `gc.roots` from
  ...
  until then. (temen already proved the cross-vCPU `gc.roots` scan separately.)
- **No JACL program codegen yet.** Phase 1 tests use **C-driver "programs"**
  (compiled via temen-llvm, like the spikes) standing in for Phase 2's emitter.
...
- **Goal:** a repeatable build that turns `runtime/*.c` into one IR artifact, and a
  test harness that `temen_ir::link`s a C-driver program against it and runs on
  interp + JIT.
- **Deliverable:** `runtime/build.sh` (→ artifact) + a Rust test harness promoted
  from the spikes (`spikes/temen_*` are the prototypes).
...
- **Goal:** `JaclVal` for the temen world — 8-bit tag + 56-bit payload; heap pointers
...
- **Depends on:** P1.1.
- **Note:** Spike-2 found temen-llvm rejects a *constant* `ptrtoint` of a global —
  route heap-base→int through a `noinline` helper (or revisit as an temen-llvm ask).
...
- **Block-local SSA** (Spike-1) is a *Phase 2* codegen obligation; irrelevant to the
  C runtime (temen-llvm handles it).
...
- **`llvm-18-dev`** is required in the build env for `temen-llvm` (libLLVM linkage).
...
**Carried follow-ups (none blocking Phase 2):** the `gc.roots` payload-mask temen
enhancement (clean tagged-root finding); `__vm_map` window growth (needs the
powerbox harness); the standalone `temen-llvm-translate` CLI + name→index export map
...
- **P1.0 harness ✅** — `runtime/harness` (Rust): compiles a C driver + runtime via
  `clang -O2 → temen-llvm`, runs `run` (func 0) on interp + JIT, asserts agreement.
...
- **P1.4 strings ✅** — `runtime/string.c` (+ `jaclrt.c` unity): inline (≤7B, value),
  ...
  **runtime root set** (the intern table) the collector marks via
  ...
  Findings: temen-llvm provides `memcpy`/`memset` but **not** `bcmp`/`memcmp`/`strcmp`
  — hand-roll byte compares in runtime + tests. And clang merges chained i64 tag
  compares into an **i64 `switch`** that temen-llvm can't lower (only i32) — runtime
  **tag dispatch must switch on the i32 type index** (`jaclrt_type_index`), not
  chained `(v & TAG_MASK) == const` tests. (Candidate temen-llvm enhancement: i64
  `switch`/`br_table`.)
...
- **P1.5 collections (vector) ✅** — `runtime/collections.c`: the real `lib/rrb_vec`
  ...
  **Key finding — `gc.roots` vs tagged values (an temen ask).** JACL stack roots are
  **tagged JaclVals** (`VECTOR<<56 | offset`, …) whose full word sits far above the
  ...
  the tight range and temen returns masked offsets — file as an temen enhancement before
  Phase 2 / production.
