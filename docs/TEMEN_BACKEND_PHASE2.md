# JACL on TEMEN — Phase 2 scope: program codegen

> Status: **scoping, pre-implementation.** Branch: `temen-backend-phase2` (stacked on
> `temen-backend-phase1`). Companions: `TEMEN_BACKEND_DESIGN.md`, `TEMEN_BACKEND_PLAN.md`
> (§4 Phase 2), `TEMEN_BACKEND_PHASE1.md` (the runtime this codegen calls into).
>
...
> JACL **programs** compile to TEMEN IR and run against that runtime.

## Goal

Retarget JACL's codegen: **AST → TEMEN IR** (via a JACL-owned IR builder), emitting
...
- **Runtime** = Phase-1 C → `temen-llvm` → an TEMEN IR artifact (exports `jacl_*`).
- **Program** = JACL AST → the IR builder → an TEMEN IR module that calls runtime
  functions via `call.import "jacl_add"` etc.
- **Link** = `temen_ir::link([runtime_unit, program_unit])` → one re-verified module.
- **ABI:** both producers must share the temen-llvm calling convention — **the data
  stack pointer is threaded as the leading parameter** of every function (temen-llvm
...
## Prerequisites — the temen-side asks (P2.0) — **DONE** (temen pin `a7cbcf1`)

Separate-artifact linking + tagged-root GC needed three temen changes (the runtime
itself unchanged). All three landed in temen (`docs/TEMEN_PHASE2_ASKS.md`) and are
wired up + verified on the JACL side:

1. **`temen-llvm` name→index export map.** ✅ `Translated.exports: Vec<(String, u32)>`
   names each defined function so the runtime artifact's `jacl_*` exports populate
   `temen_ir::link`'s `LinkUnit.exports`.
2. **`temen-llvm-translate` CLI.** ✅ `runtime/build.sh` now runs `clang -emit-llvm`
   then `temen-llvm-translate` to emit `build/jaclrt.temen` (TEMEN-IR module) +
...
3. **`gc.roots` payload-mask parameter.** ✅ `heap_gc.c` passes `JACL_PAYLOAD_MASK`
   with the tight heap range; temen strips the tag byte and returns bare offsets, so
...
- **Goal:** an in-memory TEMEN IR module/func/block/inst representation, mirroring
  `temen-ir`'s shape, with a **text serializer** now and a **binary (`temen-encode`)**
...
- **Validates:** ✅ `runtime/harness/tests/irbuilder.rs` — the builder's output for
  ...
  Import-free output is asserted to be canonical temen-text.
...
- **Deliverable:** ✅ `codegen/codegen.{h,c}` — `temen_codegen_program(nodes, count)`
...
- JACL procs → TEMEN IR functions (data-SP ABI); args/returns, recursion, arity.
- Tail calls via `return_call` where applicable.
- **Done:** top-level `proc name {params} (rettype)? {body}` → one TEMEN function each,
...
- **Done:** **strings** — a string literal's bytes go in a `data` segment and
  ...
  `temen_ir::link` so the address const is patched to the linked data address. **Vectors**
...
- ~~Emit the `gc_epoch` safepoint poll at loop back-edges + call sites.~~ **Superseded:**
  the TEMEN backend uses a **stop-the-world, conservative, non-moving** collector — no
...
- **Bring-up findings (divergences the diff surfaced).** Comparing against the old VM
  ...
  - `[length vec]` — the old VM's `length` is string-only; the TEMEN runtime's `jacl_len`
...
- **temen-llvm gaps to respect in *generated* IR** (from Phase 1): i32 (not i64)
