...
>
> Phase 2 (sequential codegen) is merged. Phase 3 makes JACL **concurrent** on temen's
...
> - **P3.5 host I/O — DONE** (temen bumped to `1737fe7` for the frontend-independent powerbox
>   reactor; `docs/TEMEN_POWERBOX_ASK.md` delivered). `print` works end-to-end, interp==jit:
...
>
> **Concurrent GC — SOUND, jobs are ordinary GC objects (temen pin `57e20b1`, incl. vm#217).**
...
> `docs/TEMEN_PHASE3_ASKS.md`):
> - **temen scans the native side** — parked fibers' control stacks (the fiber switch spills all
...
>   (`mark_task_stacks`) — temen-llvm puts address-taken locals (`parallel`'s `futs[]`) on those
...
`race`, async (parking) builtins — onto temen **fibers** + the M:N scheduler, matching
...
## The fiber primitive (Spike-3, `spikes/temen_fiber_generator`)

temen exposes `cont.*` to C as:
...
ABI: temen-llvm threads `sp` as the leading parameter, and resume passes `(fiber_sp, arg)`,
...
- A proc whose body contains `yield` is a **generator**: it lowers to an TEMEN function
...
- **Limits (current):** a generator takes ≤1 param (the single resume arg); the
  ...
  single-thread model: temen's `gc.roots` scans suspended fibers' stacks — see the temen
...
- Single-thread: the cooperative scheduler (P3.1–P3.3) runs all fibers on one OS thread,
  so GC is already a correct STW collection — temen's `gc.roots` scans **every** suspended
...
> exposes the pure-guest STW barrier (temen GC.md §2.1) over the fiber scheduler:
...
> temen's own cross-vCPU STW test (`crates/temen/tests/gc_roots.rs::
...
> exclusion) is the right mechanism (temen's `gc_quiesce.rs`); it just has to drive
> fiber-suspend, not block the bare worker.

> **Unblocked (temen `7a82f64`, PR #79).** The per-vCPU allocator needs each worker
> to know its own vCPU id cheaply, anywhere. temen shipped the ambient per-vCPU TLS
> register we asked for (`docs/TEMEN_PHASE3_ASKS.md` — RESOLVED): C intrinsics
...
> throwaway global allocator. temen already provides the rest (`__vm_thread_spawn`/
...
fast path** — adapted to TEMEN's simpler collector. *Not* a per-allocation lock.
...
- **Worker identity via the per-vCPU TLS register.** Each worker indexes
  `per_worker[id]` where `id = __vm_vcpu_tls_get()` — the ambient per-vCPU word temen
...
- **GC: cooperative STW, conservative, non-moving (extends P2.9/P3.6).** Workers reach a
  ...
  futex barrier (reference: temen's `gc_quiesce.rs`; no new temen primitive — "quiescing the
  N threads quiesces the M fibers"), then conservative `gc.roots` scans every thread's
  and fiber's stack (temen already does), sweeps each per-worker heap, and resumes. No
...
collector roots, during a mid-batch collection: every parked/suspended task fiber (temen
...
**Concurrent GC — SOUND; jobs are ordinary GC objects** (no registry, no cap, no leak — temen pin
`57e20b1` incl. vm#217; see the top-of-doc contract note and Ask 3 in
`docs/TEMEN_PHASE3_ASKS.md`). `jacl_sched_mark_roots` = window (queues, `running_job`, root job) +
a conservative scan of the in-use fiber DATA stacks (`mark_task_stacks` — where temen-llvm puts
...
- **Interpreter under heavy concurrent GC.** The temen interp's cooperative scheduler still
...
The temen powerbox was never the blocker — it is complete in temen. The earlier obstacle
(hand-building the powerbox stash/entry layout for JACL's hand-linked codegen module, which
faulted with `MemoryFault`) is **resolved** by temen `1737fe7`'s frontend-independent
reactor (PR #109 + #118 + the F1–F6 `Instance` refactor): `temen_ir::synth_powerbox_start`
(generalized powerbox-entry synthesis over any linked module — it lays out the writable
stash + memory), `temen_run::instantiate` / `instantiate_with_imports` (wasm-style name-keyed
import binding), and `Instance::run_diff` / `call` / `Session::call_export` (call by name,
interp==jit enforced). This was exactly `docs/TEMEN_POWERBOX_ASK.md`.
...
- **Runtime `jacl_print`** (`io.c`): value text + newline to stdout via libc `write` →
  ...
  reassociate it into a sparse switch temen-llvm rejects.)
...
- **Async (parking) builtins** (`sleep`, channels): the io_ring submit/complete capability
  exists in temen, but builtins that **park** the fiber on completion need the deferred
