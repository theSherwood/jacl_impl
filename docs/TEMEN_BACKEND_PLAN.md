# JACL on TEMEN — Migration Plan

> Status: **plan, pre-implementation.** Companion to `TEMEN_BACKEND_DESIGN.md` and
> `TEMEN_GC_CONTRACT.md`. This is an **all-or-nothing backend swap**, so it lives on
> its own branch (`temen-backend`) and does not disturb `main` until it can pass the
> existing test corpus on the new backend.

## 0. Where things stand

- **temen side: ready.** `vendor/temen` @ `7737c9e` ships `gc.roots` + the `ref`
  `ValType`; verified (`TEMEN_GC_CONTRACT.md`). The GC interface JACL needs exists.
- **Runtime path: LLVM, not chibicc.** The runtime is C compiled via **`temen-llvm`**
  (clang `-O2` → LLVM bitcode → TEMEN IR), which is production-mature (8 real libs
  ...
  JACL-emitted program IR via temen static linking (`temen_ir::link`, `DYNLINK.md`).
  (See Design §4.4/§4.5.) chibicc is dropped (spec/oracle only).
- **The one concrete temen-side ask: SATISFIED** (temen `f75509a`, slices AC/AD/AF).
  `temen-llvm` now lowers the full `<temen.h>` `__vm_*` surface — **35 builtins** incl.
  ...
  matches the ask exactly. (Tests run in temen CI; not runnable in this sandbox —
...
- **Behavior parity is the bar:** success = the existing JACL test corpus passes
  on the TEMEN backend (modulo the explicitly-allowed differences, e.g. GC
  occupancy per R1).
...
- **Spike-1 — program IR emission. ✅ DONE** (`spikes/temen_emit_link`, PASS).
  Emitted JACL-shaped **text IR** (incl. a real SSA loop calling the runtime),
  passed the **verifier**, **static-linked** (`temen_ir::link`) program units against
  a separate runtime unit (cross-module call resolved by name), and ran identically
  on **interp and JIT** (`sum_to(100)=5050`, etc.). Finding: temen is **block-local
  SSA** — cross-block values must be threaded as block args (now a codegen
  obligation, Design §4.4). Binary `temen-encode` emission remains a later speed swap
  behind the same builder.
- **Spike-1b — runtime via `temen-llvm` + the intrinsic surface. ✅ DONE**
  (`spikes/temen_llvm_runtime`, PASS). (a) JACL's **real `lib/rrb_vec/rrb_vec.h`**
  (RRB persistent vector) compiled clang `-O2` → `temen-llvm` → TEMEN IR, verified, and
  ...
  by temen-llvm's own `vm_*` tests (12/12). Confirms "port + recompile the runtime,
  don't re-author it." Needs `llvm-18-dev` in the build env.
- **Spike-2 — GC model. ✅ DONE** (`spikes/temen_gc`, PASS). A conservative,
  non-moving **mark-sweep** over a window heap, roots via the real `gc.roots` op,
  compiled through temen-llvm and run on **interp + JIT**: the live stack root is
  ...
  here); temen-llvm rejects a *constant* `ptrtoint` of a global (route through a
  `noinline` helper). The riskiest design point holds end-to-end.
- **Spike-3 — fibers replace SM. ✅ DONE** (`spikes/temen_fiber_generator`, PASS).
  ...
  **deep yield** (`suspend` from a `noinline` nested call, which the SM transform
  can't express), and **two interleaved live fibers** — compiled via `temen-llvm`,
...
> capstone. Full step record + findings: `TEMEN_BACKEND_PHASE1.md`.

> **Scoped into concrete, ordered steps in `TEMEN_BACKEND_PHASE1.md`** (P1.0–P1.7,
...
- Stand up the **runtime build**: existing/ported runtime C → clang `-O2` →
  `temen-llvm` → IR artifact; static-linked against program IR (Design §4.5).

## 4. Phase 2 — codegen retarget

> **Scoped into concrete, ordered steps in `TEMEN_BACKEND_PHASE2.md`** (the JACL-owned
> IR builder, the two-producer/link model + data-SP ABI, the temen-side prerequisites,
> P2.0–P2.10, exit criteria). Largest phase; expect sub-phasing. Summary:

- Rewrite `compiler.c` emit sites: AST → TEMEN IR (keep the type-aware structure).
- Insert `gc_epoch` safepoint polls at back-edges/call sites (one poll, shared
  with temen's epoch check).
...
- Run the existing C test corpus + `test/jacl/*.jacl` against the TEMEN backend.
...
- Only when the corpus is green on TEMEN: make TEMEN the default backend, retire the
  bytecode VM (`vm.c` dispatch, `bytecode.c`), the SM transform, and the epoch GC.
- Merge `temen-backend` → `main`.
...
| Codegen (`compiler.c` minus SM) | ~18k | **retargeted** to TEMEN IR |
| Builtin/runtime semantics (from `vm.c`) + `lib/` collections | ~12k+ | **ported + recompiled** via `temen-llvm` (substrate swaps, not re-authored) |
...
- **R-temen-llvm-intrinsics — RESOLVED** (temen `f75509a`): the on-ramp now lowers the
...
- **R-Runtime-port (med):** existing runtime C needs substrate swaps (window
  ...
  some C may hit temen-llvm gaps (e.g. varargs `printf`, not yet supported).
...
1. Program IR emission (Spike-1) — text-then-binary behind one builder; confirm
   `temen-text` reachable and `temen_ir::link` merges program + runtime modules.
2. Runtime packaging — granularity of the `temen-llvm` artifact(s) and the
...
4. How JACL's sandboxed `interpret` maps onto TEMEN domains/capabilities.
