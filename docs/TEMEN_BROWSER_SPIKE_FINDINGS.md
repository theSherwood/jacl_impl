# TEMEN-in-browser: re-measuring the blocker

> **Status:** experiment results. Corrects the root-cause diagnosis in
> `TEMEN_BROWSER_PLAN.md` §"Spike results / B". Same goal: move the playground off
> the old bytecode VM onto the TEMEN backend. Tooling: `runtime/harness/src/bin/probe_temen.rs`
> (new) and `jacl_temen --bytecode`.
...
With both slices in the vendored temen, `bytecode::compile_module` **accepts** the linked
module (no env hack) and `jacl_temen run --bytecode` prints `hi` / `1\n2` on the bytecode
...
`bash runtime/build.sh` bakes `jaclrt.temen` (clang-18 + temen-llvm). `probe_temen`
mirrors `emit_temen`/`jacl_temen` up to `synth_manifest_start`, then instead of
encoding it **measures** the linked module:

1. `bytecode::compile_module` verdict (the browser wasm-safe engine, all-or-nothing).
2. `temen_opt::optimize_module` (devirt + inter-fn DCE) — does it prune?
3. Per-instruction declined-op census + reachability from `_start`.
4. `temen_wasm_jit::analyze` mixed-tier verdict (`mixed_ok`).
...
- **The blocker is not op coverage.** The bytecode engine already lowers everything
  ...
  are all already supported arms in `temen-interp/src/bytecode.rs`). The reachable
  declined-op count is zero.

- **One veto gates all JACL programs.** `compile_module_with`'s module-level seam
  veto (`temen-interp/src/bytecode.rs`, the `has_gc && has_thread` term) declines any
...
- **Tree-shaking cannot sidestep it.** `temen_opt::optimize_module`'s inter-function
...
The single-vCPU powerbox (`compile_and_run_with_host`, what `temen_run_pb` uses) traps
...
`gc.roots` across all vCPUs). `probe_temen`'s `run_compare` tests both, with `write`
...
during startup. The tree-walker services it (`temen-interp/src/lib.rs:9183`, where the
...
(`temen-interp/src/lib.rs:10046`), because the pure evaluator has no vCPU context. Same
...
spawned thread to its index), and a differential test (`crates/temen-interp/tests/vcpu_tls.rs`)
pins it to the tree-walker. Full temen-interp suite green.
...
tree-walker's documented scope (`crates/temen/tests/gc_roots.rs`), a sound superset
...
(`crates/temen/tests/gc_quiesce.rs`), and JACL's roots live in the migratable fibers the
shared registry covers. `drive_parallel` uses a per-thread fiber set, so there is no
cross-thread race. New tests (`crates/temen/tests/bytecode_gc_roots.rs`) pin that `gc.roots +
thread` and the JACL-shaped `gc.roots + thread + parked-fiber` run natively and stay sound;
full temen-interp + temen gc/concurrency suites green.

## End state — JACL runs on the wasm-safe engine

With both slices in the vendored temen (submodule bumped to the PR head), no env hack:

```
$ bash runtime/build.sh && cargo run --bin jacl_temen -- run --bytecode prog.jacl
# print "hi"  → hi
# generator   → 1 / 2
$ cargo run --bin probe_temen -- prog.jacl
...
2. **The browser run path is wired** (`demo/temen/`, `demo/src/temen-jacl-wasm.ts`): a JACL
   `.temen` runs through the `temen-browser` cdylib's `temen_run_onramp` on `wasm32` and the
   captured stdout comes back as the playground's existing `RunResult`. Proven headless —
   `print "hi"` → `"hi\n"`, a generator → `"1\n2\n"`, `temen_status=0`. What's left is the
   ...
   (`TEMEN_BROWSER_PLAN.md` option (b)); precompiled examples run today.
3. **Independently useful:** narrow `dead_func_elim`'s indirect-dispatch gate (bails on
   ...
   and helps the `temen-wasm-jit` mixed tier.

## Reproduction

```sh
bash runtime/build.sh                                   # -> runtime/build/jaclrt.temen
cd runtime/harness
cargo run --bin probe_temen -- /path/to/prog.jacl        # measures + runs the linked module
cargo run --bin jacl_temen -- run --bytecode prog.jacl    # runs on the wasm-safe engine
```

No engine patch or env var is needed: the vendored temen carries the `vcpu.tls` lowering and
the veto removal.
