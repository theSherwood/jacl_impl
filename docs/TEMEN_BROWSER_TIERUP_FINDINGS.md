...
**Status: RESOLVED on the temen side (defect diagnosed + fixed); adopted as temen's WASM_AOT.md
"slice 0".** The confirmed defect is fixed in temen `main` and is present as of our submodule pin
`bf75dd72` (temen PR: "slice 0: gate page ops in compile_module_tierup", commit `17e06a4e`). The
sections below are the original postmortem, preserved for context; the resolution is summarized
next.

**Update (temen pin `63a70532`, latest).** The *entire* WASM_AOT slice plan has since landed — slice 1
...
reactor (`temen_warm_open`/`_eval`/`_close`). For a *page-managing* guest like the JACL compiler, the
sanctioned route is the warm-snapshot's own trick: map a large-enough window (temen uses
`WARM_MAPPED_LOG2` = 2²⁶) so the heap never `vm_map`-grows — no page ops → simultaneously JIT-eligible
**and** snapshot-restorable. That is the large/fixed-window lever noted elsewhere, now proven by temen
(QuickJS, an on-ramp of the same shape as our compiler-guest, went from a >1 s do-nothing floor to
~2 ms warm). Nothing further is blocked on temen; the remaining work to close the guest-JIT↔AOT gap is
JACL-side wiring (a two-phase `warmup`/`eval_run` driver over a pre-sized window). See
`vendor/temen/WASM_AOT.md`.

## 0. Resolution (temen side)

temen reproduced the defect natively and fixed it. Which of this doc's ranked hypotheses held:

- **Hypothesis 1 (data/window materialization) — ruled out** for the native path. A new harness
  (`temen-wasm-jit/tests/tierup_live_window.rs`) drives the real loop — a `bytecode::VcpuReactor`
...
temen has started (`temen_ir::bounds` + "wasm-JIT elides the bounds-trap on a provable in-window
...
real performance story, and this stays a latent option, not a live need. One residue temen flags as
unverifiable without a node+cdylib browser harness: whether the `temen_par_run`/`PAR_TIERUP` lane's
`win`/`size_log2` provenance matches `temen_run_onramp` for a guest that *grows* its window mid-run —
moot in practice now that the gate routes such guests to the interpreter.

---

References below are to the `vendor/temen` engine (`crates/temen-wasm-jit`, `browser/src`). Line
numbers are approximate — read against current temen `main`; the symbol names and module-doc
sections are the stable anchors.

## 1. Motivation

The playground's macro-capable live path can run the self-hosted JACL compiler **as an TEMEN
guest** (`jacl_compiler.temen`) on the browser's bytecode interpreter. It is correct but slow —
...
only tiny macro *bodies* on TEMEN — see `docs/TEMEN_MACRO_STAGING_PLAN.md` and
`demo/temen/README.md`. This spike was the *other* direction: make the guest itself fast.)
...
## 2. What temen offers (baseline)

`temen-wasm-jit` has two JIT entry points:

- **`compile_module`** — whole-guest wasm JIT. Gated by `compile_tier_eligibility`
  (`crates/temen-wasm-jit/src/lib.rs`, the `TierEligibility` struct, ~L819): every *reachable*
...
- **`compile_module_tierup`** — per-function tier-up. Emits `f{i}` for eligible hot functions;
  ...
  **over the same shared window**. This is the path built for `thread.spawn` bodies: the main
  ...
  marshals `argv`, a Worker runs `f{func}`, and `temen_par_deliver_tierup` returns the results.
  The whole thing is driven by the resumable `temen_par_run` event loop.

### The load-bearing invariant

Every emitted memory access replicates trap-confinement masking **with constants baked at emit
time** (`temen-wasm-jit/src/lib.rs` module docs, ~L22–33 and the `compile_module_tierup` body,
...
via `env.call_interp`, and drive it through `temen_par_run` + `VcpuEvent::TierUp`.
...
1. **Window / data-materialization mismatch (most likely).** The `thread.spawn` tier that
   ...
   `win` base the interpreter has been using. If the `temen_par_run`/`TierUp` setup hands the
   ...
   **Check:** does the onramp/`temen_par_run` tier-up path write `m.data` into the shared window
   and pass the *live* `win` + `size_log2`, the way `temen_run_onramp`'s interpreter init does?
...
## 6. Diagnostics for the temen follow-up
...
- **Diff the window setup** between `temen_run_onramp` (interp) and the `temen_par_run` / `TierUp`
