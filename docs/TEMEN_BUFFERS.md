# Buffers & pointers on the TEMEN backend

How `[Buf N T]`, `[Ptr T]`, `addr`, and the `ptr-*` operators are lowered by the JACL→TEMEN
...
Early TEMEN-backend work modelled a buffer as a **`JaclVal` array** (the mutable-array runtime
...
**This was an implementation shortcut, not an TEMEN constraint.** The JACL heap *is* TEMEN
...
- **Raw memory semantics.** `addr` of a scalar, cross-buffer `ptr-diff`, reading a
  ...
  a byte-addressable buffer. These need a linear-memory-backed buffer (an temen `memory`
...
These are the buffer/pointer entries in `docs/TEMEN_PARITY_NOTES.md`'s difficulty map; the
