# Follow-up for temen — fiber timed-wait contract (PR #442), from the jacl consumer
...
`TEMEN_INTERP_TIMED_WAIT_REGRESSION.md` — thank you. jacl has adopted the new contract (a
`memory.wait` inside a fiber reports `FIBER_PARKED` at `cont.resume`, and the resumer
re-polls). The 8 sleep/concurrency parity cases pass `interp == jit` again with temen pinned at
...
`temen-llvm`'s `__vm_fiber_resume` lowering stores the `cont.resume` status straight into the
C `int *done`, but its comment still reads:

```rust
// crates/temen-llvm/src/lib.rs  (__vm_fiber_resume)
...
fiber (and, for temen's *own* scheduler, at scheduler idle). That is the right default, but it
leaves a hole for a **guest scheduler that owns the loop on its vCPU**: temen's "fire at
scheduler idle" path only triggers when temen's vCPU task is itself non-runnable, which never
...
- **busy-spin `cont.resume`** (temen's own `poll_loop` test kernel does exactly this) — the poll
...
run (it knows that), so temen still never has to understand guest scheduling, and the
...
- Original regression report: `docs/TEMEN_INTERP_TIMED_WAIT_REGRESSION.md`.
- The temen contract + tests: `crates/temen/tests/fiber_timed_wait.rs`,
  `crates/temen-interp/tests/fiber_parks.rs`.
- jacl's adoption: `runtime/sched.c` (`resume_job` / `worker_loop` `FIBER_PARKED` handling),
  this branch (`claude/temen-refresh-manifest-migration`, temen pinned at `a45d80cc`).
