...
`append-file`) and the temen side is ready.
...
**temen side** (as of the refreshed submodule) already implements the layered capability
story we need, so this design is mostly *adoption*, not invention:

- **`crates/temen-fs`** — the fs-capability **wire protocol** (ops 0–19: open/read/write/
...
- **`crates/temen-run/src/fs.rs`** — the **real-filesystem backend** (`host_fs(root)`),
...
- **Guest access from C** — `__vm_cap_resolve("fs", 2)` → handle (negative when not
  ...
  (probe once, cache, fall back on not-granted) is the same one temen's own
  `fs_probe.c` fixture and the sqlite/lmdb/postgres demos use.

**What temen-posix is — and why it is *not* our tool.** `crates/temen-posix` is a *POSIX
...
where only `fs` is wanted would widen the authority for nothing. We use `temen-fs`
directly and leave `temen-posix` for the day someone wants to run a foreign C tool
...
```
  ...
  runtime adapter         io.c: probe __vm_cap_resolve("fs") once at init
        │                   granted  → route builtins through cap ops (temen-fs protocol)
...
temen-fs protocol refuses absolute paths by design: the capability is the root, so paths
...
- `..` and empty components are refused by *both* temen-fs backends (`-EACCES`); the
...
1. Every grant **clones the backend's seed fresh** (the `make` builder), so each leg
   sees a pristine filesystem — documented and implemented in temen-fs.
...
```
program module (TEMEN IR, linked against the runtime)
...
| platform | grant |
|---|---|
| dev CLI (`jacl_temen run`) | explicit `--fs-root DIR` (`host_fs`, `DIR/tmp` pre-created) or `--fs-image FILE`; **no flag = no grant** (default-deny — rule 1 wins over convenience) |
| test / parity harness | `mem_fs` seeded with `dirs=["tmp"]` (and, dual-mode, no grant at all — both must pass) |
| browser / wasm | `mem_fs_seeded` from the bundle's data image (temen-fs builds for wasm; no real fs exists) |
...
- **`exec`** (would unblock `io_pipe_to_file` and `tour`'s shell section): a named cap
  ...
  note in `TEMEN_PARITY_NOTES.md`.
- **`clock`, `net`, …**: same shape. `temen-run` already exposes `HostCap::clock()`.
...
2. **Harness dual-mode — DONE** (`parity.rs`): every `io_*` case runs twice — plain
   ...
   falling back.
3. **CLI + bundle tooling — DONE** (`runtime/harness/src/bin/jacl_temen.rs`):
   ...
   `jacl_temen bundle <assets-dir> -o <out.fsimg>` walks a host directory into an
   ...
   ride temen-encode when a consumer needs it.
4. **Surface growth — DONE for the first tranche.** `delete-file` (op 5),
   ...
   and the TEMEN runtime (guest-VFS model over the map + cap path), driven by corpus tests
   (`io_delete_exists`, `io_list_dir`) that the harness's dual-mode gate runs granted and
   ungranted. The `exec` capability ask is written up in `docs/TEMEN_EXEC_ASK.md`.

## Non-goals

- Reproducing host-`/` semantics inside JACL (the rooted namespace is the point).
- temen-posix adoption (libc personality; wrong granularity for a runtime we own).
