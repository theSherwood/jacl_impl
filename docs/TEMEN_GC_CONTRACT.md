# TEMEN GC Support Contract (conservative, non-moving)

> **Status: CONVERGED — temen side landed & verified.**
> temen implemented this contract as of `vendor/temen` @ `7737c9e`
> (`GC.md` on the temen side is the mirror). Verified here on 2026-06-15:
> `cargo test -p temen --test gc_roots` → **8/8 pass**, including suspended-fiber
...
>
> This document is JACL's copy of the interface. temen's `GC.md` is authoritative for
> temen-side semantics; keep the two in sync.
...
**temen implements no GC and no object model** — it provides only fiber **root
...
- **temen provides:** enumeration of roots living on **control stacks +
...
Handshake is **pure guest code** over temen's window-atomics + `memory.wait/notify`
...
signals done). **No new temen world-stop primitive required.**

## The temen primitive: range-filtered root enumeration
...
temen walks every fiber **not actively executing guest mutator code** (all parked
...
1. **Range-filtered** — only in-window candidates cross the boundary; host return
   addresses / frame pointers / host pointers are filtered *inside* temen. No host
   ASLR/layout leak.
...
4. **Mechanism, not policy** — temen does the window range-check; JACL does
...
- **R2 — temen scans the `gc.roots` caller too** (the call is the collector's own
  safepoint; temen reads the caller's out-of-band control stack on its behalf).
- **R3 — Quiescence is a caller precondition** temen can't cheaply enforce; sound
  only under STW. Optional temen assert: refuse if another vCPU holds a RUNNING
  fiber.
...
1. **Safepoint polls** at loop back-edges + call sites — no un-polled tight loops
   (else STW stalls; temen has no async escape hatch by design). Piggyback the
   `gc_epoch` poll on the same sites used for temen's epoch check.
...
3. JACL scans its own **data stack + heap + GC structures**; it relies on temen
...
### JIT "spilled-only" contract (temen GC.md §7)
...
shim is a noted temen follow-up). **Fully covered under cooperative STW** — every
...
- **Reserved now:** the `ref` `ValType` (opaque 64-bit, lowers as `i64`) — already
  in temen. Prevents a format break when precise stack maps arrive.
