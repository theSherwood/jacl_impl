...
Scopes the final Phase 6 work (`docs/TEMEN_MACRO_STAGING_PLAN.md` §6.4): letting the
JACL compiler **compiled to TEMEN** (`jacl_compiler.temen`) expand macros without the
legacy VM, by codegen'ing each macro body to TEMEN IR and running it *in-guest* via
TEMEN's guest-JIT capability — and, per the same mechanism, re-hosting `interpret`.

This is a **scoping doc**, not an implementation. It records the target TEMEN API, the
...
> failure mode, in `docs/TEMEN_BROWSER_TIERUP_FINDINGS.md`.

## 1. Why

Two things still pin `vm.c` into the toolchain (Phase 6.1–6.3 removed the rest):

- **Macro expansion in the `.temen`.** The `.temen` guest has no staging bridge (it
  can't link the native Rust `jacl_temen_stage` staticlib — it *is* guest code), so its
  `jacl_macro_stage_hook` is NULL and macros fall back to the legacy bytecode VM
  (`vm.c`, running confined inside the guest). `docs/TEMEN_SELFHOST_FEASIBILITY.md` §2
...
Both are the same shape — *compile JACL → TEMEN IR, then run it* — and guest-JIT is the
tool that lets the guest do the "run it" half itself, inverting `interpcap.c`'s stated
premise that "the guest is temen bytecode with no JACL compiler or VM inside it."

## 2. The target TEMEN API — the `Jit` capability (§22)

TEMEN exposes two nesting mechanisms; only one takes **guest-authored IR**:

| Mechanism | cap_id | Runs | Confinement |
|---|---|---|---|
| **`Jit`** (§22) | **11** | **guest-emitted, pre-encoded TEMEN IR bytes** | **same domain** — same window + powerbox, re-verified, *no* nested sandbox |
...
**Guest ABI** (C intrinsics, the 8th powerbox cap; `frontend/chibicc/include/temen.h`):
...
`temen-run/src/lib.rs`). No `RunConfig` toggle, no opt-in — running under the powerbox is
...
**Reference implementations** to copy: `crates/temen-run/demos/jit/jit_demo.c` (hand-emit
`temen-encode` bytes → compile → invoke) and, closest to us,
`crates/temen-llvm/tests/fixtures/peval_futamura/` (build an `temen_ir::Module` in guest
memory, `temen_encode::encode_module`, `__vm_jit_compile`, `invoke`).

## 3. The gaps on the JACL side

Three are hard blockers; the rest are plumbing.

### 3a. Codegen emits temen-**text**; `Jit` wants temen-**encode binary** (blocker)

`irbuilder` only has `irb_to_text` (`codegen/irbuilder.h`); the header already
anticipates the fix — *"A binary (temen-encode) serializer can later sit behind the same
API."* The native bridge sidesteps this by parsing the text in Rust
(`temen_text::parse_module`). In-guest there is no text parser.

**Work item:** add `irb_to_encoded(const IrModule*, size_t *out_len)` — a C serializer
emitting the `temen-encode` binary format (LEB128 + one-byte opcodes; magic `TEMEN\0`,
...
`temen_encode::decode_module` == the text-parsed module, over the whole corpus.

**Non-obvious sub-problem (scoped):** temen-text writes `call.sym "name" (sig) …` with the
...
order* `temen_text::parse_module` assigns them — otherwise the decoded module's `CallSym`
...
precondition to accept read-only data segments that fit the window — an **TEMEN-side**
...
module. The `.temen` does **not** currently link jaclrt at all (it's the *compiler*, not a
runtime).

**Confirmed symtab wire (was §6 open question).** `compile_linked`'s symbol table is
**not** `{name → address}`. It is (`temen-run::decode_symbol_table`, LEB128 mirroring
`temen-encode`): `count`, then per entry `name` (uleb len + UTF-8), a `kind` byte, and a
...
of the compiler guest's image — linked into the `.temen` so `jacl_vec_push`/`synrt_*`/… are
...
(0..n_real_funcs) | padding slots]` (`temen-jit::CompiledModule::fn_table`), so a module
function's funcref index **is** its slot. temen-llvm lowers a taken function address to its
...
- ✅ *Symbol separation* (was the hard blocker). The compiler `.temen` already embeds the
...
- ⏳ *Entry ABI (the current edge).* `__vm_jit_invoke2(code, a, b)` passes **exactly 2** entry
  params (`temen-run::jit_invoke_locked` rejects any other arity), but `temen_codegen_staged_macro`'s
...
- ⏳ *The hook + wiring.* A `jacl_macro_stage_hook` variant that does `temen_codegen_staged_macro`
  ...
  the hook into `build_compiler_temen.sh` and differential-test against the golden corpus.
...
`temen_codegen_staged_macro` already produces an arity-1 `(sp, resume)` shape; the
...
Only the compiler `.temen` links `codegen.c`. A normal program that calls
...
the TEMEN-native compiler + a runner, replacing today's host-legacy-VM backend. Programs
...
- **(A) Host cap, TEMEN-backed (recommended default).** Keep `interpret` a host cap, but
  swap its backend from the legacy VM to the TEMEN engine: the host compiles the source
  with the native compiler and runs it on a fresh `temen_run` instance (exactly the
  `jacl_temen` / `jacl_temen_stage` path). **Drops `vm.c` from `interpret`, preserves full
...
- **(C) Guest-compile + `Instantiator`-confined.** Compile in-guest, then run the module
  ...
  host-granted modules), so this needs an **TEMEN feature** (accept a re-verified
...
in-guest — either the `interp` cap is a **compiler-as-a-capability** (the TEMEN-native
...
1. **`irb_to_encoded` (temen-encode binary serializer in C)** — §3a. Standalone, testable
   by round-trip against `temen_encode::decode_module`. Unblocks everything and helps the
   native path too. *Prereq for all guest-JIT work.*
2. **Data-segment-free literal lowering** under a `staged_macro` codegen flag — §3b. **DONE.**
   With 3+4 landed, a data-*free* macro (e.g. `id`) expanded in-guest end to end, but a macro
   with string/symbol literals still emitted a `data` segment the `Jit` cap rejects
   (`jit_resolve_and_validate`: `!m.data.is_empty()`). `compile_string_literal` now honors a
   `cx->data_free` flag (set from `in_guest`): instead of `irb_add_data` + `jacl_str_new(ptr,
   len)`, it builds the string from **immediate byte constants** — `synrt_str_empty()` then one
   `synrt_str_push_byte(s, b)` per byte (the glue helpers do the pointer-based construction in
   the compiler, so the macro object carries no `data.self`). The native path (`in_guest=0`,
   `data_free=0`) is unchanged and keeps the data-segment lowering. Proven on the TEMEN engine:
   `twice 21` (whose `syntax-quote [+ ~x ~x]` body has the `+` symbol literal) → `[+ 21 21]`
   in-guest (`build_compiler_temen.sh --selftest` STAGETEST). This unlocks the string-bearing
   corpus in-guest.
3. **Staging runtime in the guest image + symtab for `compile_linked`** — §3c. **DONE.**
   jaclrt (+`syn_rt`+`stage_glue_guest`) is linked into the `.temen` (`jrtg_`-renamed to
...
4. **Guest-JIT staging hook** — **DONE** (`stage_bridge_guest.c`): `temen_codegen_staged_macro`
   ...
   `synw_decode`. Proven on the TEMEN engine for the **whole macro corpus** — `twice` (`[+ 21
   ...
   golden (`build_compiler_temen.sh --selftest` STAGETEST, one translate+run over all five).
5. **`interpret` via guest-JIT (model B)** — reuses 1–4, generalized from a macro *body* to a
   ...
   **in-guest interpret hook** (`jacl_install_temen_interpret_hook`, `interpret_bridge_guest.c`) —
   parses SRC, codegens a runnable module (`temen_codegen_program`, new `in_guest=1` inline entry:
   ...
   in-guest (`build_compiler_temen.sh --selftest` INTERPTEST). **Slice 2 DONE** — macro expansion
   ...
   (`grant_jit_fibers`, now supported by the engine — temen §22).
6. **Drop `vm.c`** — once 4 (staging) and 5 (interpret) land, `src/jacl.c` stops needing
   ...
   `jacl_ctx_run_closure` / `OP_SYNTAX_OP`, and verify the `.temen` builds, shrinks
   (feasibility §2's ~33 dead externals go away), and the corpus still matches the golden.
7. **Make binary `card` the default codegen output** — **DONE** (this slice). The emit
   ...
   then the `irb_to_encoded` module bytes. `--text` restores the temen-text + `%%RELOCS%%`
   form for goldens/human diffs. A shared `jacl_runtime_harness::decode_emitted` decodes the
   container (`temen_encode::decode_module`, no `temen_text::parse_module` round-trip); the
   native consumers moved to it — `codegen.rs`, `jacl_temen`, `emit_temen`, `probe_temen`,
   `bench_temen`, `parity`, `browser_coverage`. The golden scripts (`run_diff.sh`,
   ...
   *Deliberately still on text (separate producers / follow-ups, not this slice):* the
   browser frontend `jacl_emit_ir` (the JS host has no temen-encode decoder yet — a
   `temen_browser` cdylib change), and the macro-staging emitter `emit_macro_body.c` /
...
- **Data segments (§3b).** Confirm no other codegen path the macro body can reach emits a
  ...
  literals, the JACL-side lowering grows, or the TEMEN-side "accept window-fitting ro data"
  option becomes more attractive.
- **Runtime image size in the guest (§3c).** Linking jaclrt into the compiler guest grows
  the `.temen`; measure against the `vm.c` bytes it removes — net should be a shrink, but
  verify.
...
- **`compile_linked` symtab format.** ✅ **Confirmed** (see §3c): LEB128 `count` + per-entry
  ...
  trailing bytes (`temen-run::decode_symbol_table`/`encode_symbol_table`). Names resolve to
