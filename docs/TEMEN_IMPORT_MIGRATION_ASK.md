# Ask for temen — a supported embedding path for a separate-artifact frontend (jacl)

**Audience:** the temen agents (`theSherwood/vm`). **Consumer:** jacl
...
runtime carrying live capability imports). jacl cannot refresh past temen `29b865dd` without
a supported replacement. This doc states jacl's shape precisely and asks for one public
entry point that serves it. **All references below are against temen `4937780f` (current
`main`).**

## Why jacl is different from the C on-ramp

The C frontend hands temen-llvm one translation unit — program *and* its runtime together —
so temen-llvm synthesizes the `_start`, sees every `write()` as a manifest import, and the
...
1. **The runtime** — `runtime/jaclrt.c`, compiled by clang and translated by
   `temen_llvm::translate_bc_path`. Its shape today (verified):
   ```
   memory 25
...
2. **The program** — jacl's own codegen (`codegen/codegen.c` → temen-IR text) emits the
...
The two are joined with `temen_ir::link([runtime, catalog, program])`, then the linked
module gets a powerbox `_start` and runs on interp+jit under the differential oracle. This
is jacl's whole backend. **This is the shape temen needs to keep supporting.**

## The four concrete breakages (temen `4937780f`)

### 1. `resolve_capability_imports` deleted — *has a clean replacement (non-blocking)*

`temen_run::resolve_capability_imports` (lower the runtime's `write` import to a `cap.call`)
was removed in `bd0e2b8b`. **Resolved on jacl's side:** `temen_ir::resolve_imports_with(&m,
|n| temen_run::default_cap_resolver(n).map(Into::into))` reproduces it (a
...
`temen_ir::synth_powerbox_start` / `_with_names` / `_for_imports` + `build_powerbox_start`
...
`docs/TEMEN_POWERBOX_ASK.md` ("expose the bootstrap independently of the C frontend"), and
phase-4 deleted them. temen-llvm still synthesizes `_start` **internally** (`synth_start`,
`crates/temen-llvm/src/lib.rs`), but that is private to the C path. A frontend that emits IR
...
entry; return"* (jacl does not seed the temen heap — the old call site passed
`seed_heap=false`). Every input it needs is already public: `powerbox_entry_sp(&linked)`
(`temen-ir` L2804), `POWERBOX_STACK_RESERVE` (L2746), `POWERBOX_HEAP_BRK/TOP`. The only thing
...
`temen_ir::link` (`crates/temen-ir/src/lib.rs` L3494) resolves each unit's **`CallSym`** imports
...
```rust
// crates/temen-ir/src/lib.rs:3642
imports: Vec::new(),
```
...
`temen_run::is_named_powerbox_entry` (L2693) requires the entry to be **function 0** (paramless
...
1. **Best — `link` preserves unresolved manifest imports.** Make `temen_ir::link` carry each
...
2. **Re-expose a frontend-neutral `_start` synthesizer** — the public successor to the
   ...
   restores the `TEMEN_POWERBOX_ASK.md` guarantee for non-C frontends. Combined with (1) it is
   the complete path.
...
3. **A one-call embedding wrapper** — `link_and_powerbox(units, entry_symbol,
   ...
   separate-artifact frontend" analog of what temen-llvm gives C.
...
- no jacl-side code reads a non-public temen layout constant or reimplements func/type
  reindexing.
...
- temen: `IMPORTS.md §3.6` (jacl consumer pinning), `bd0e2b8b` (phase-4 deletion),
  `322527ce` (by-name unification), `crates/temen-ir/src/lib.rs` L3494/L3583/L3642 (`link`),
  L2693/L2804 (`is_named_powerbox_entry`/`powerbox_entry_sp`), `crates/temen-llvm/src/lib.rs`
  `synth_start` (the private C-path bootstrap this asks to re-expose).
- jacl: `docs/TEMEN_POWERBOX_ASK.md` (the original frontend-neutral-bootstrap ask this
  regression reopens), `docs/TEMEN_EXEC_ASK.md` (the exec cap, already delivered — jacl wants
