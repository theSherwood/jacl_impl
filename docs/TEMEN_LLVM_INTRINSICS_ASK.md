# Ask for temen: expose the concurrency/GC ops in the LLVM on-ramp

> **STATUS: SATISFIED — temen `f75509a` (slices AC = P0+P1+Memory, AD = P2, AF =
> SharedRegion).** `temen-llvm` now lowers the full `<temen.h>` `__vm_*` surface
...
> For the temen side. Small, bounded request. Context: JACL is building its backend
> on temen and will compile its **runtime** (allocator, GC, scheduler, value ops,
> collections) as C → `clang -O2` → LLVM bitcode → **`temen-llvm`** → TEMEN IR, then
> static-link (`temen_ir::link`) against JACL-emitted program IR. This replaces the
> chibicc path (chibicc stays as spec/oracle only).

## The gap

`temen-llvm` is production-mature for ordinary C (the 8-lib corpus runs
byte-identical to native clang). But it currently lowers **only ordinary C** — a
grep of `crates/temen-llvm/src/lib.rs` shows **zero** handling for `cont.*` (fibers),
atomics, futex (`wait`/`notify`), threads, or `gc.roots`. JACL's runtime needs all
of these. The corresponding **IR ops already exist** in `temen-ir`/interp/JIT (and
`gc.roots` landed recently, opcode `0xEA`); they're just not reachable from the
LLVM frontend.

## The request

Teach `temen-llvm` to recognize a fixed set of **external symbols** (declared, never
defined — same idea as the chibicc `__vm_*` builtins) and lower a call to each into
the corresponding TEMEN IR op, instead of an ordinary `call`.

- **Mechanism precedent:** this is the same shape `temen-llvm` already uses to bind
  `write`/`read`/`exit` to §7 named imports and to synthesize `__temen_memset`/
  `__temen_memcpy` helpers — just matching on call target and emitting an op.
...
- **`__vm_gc_roots`** — *the one with no chibicc precedent; define it here.*
  ...
  per JACL's `TEMEN_GC_CONTRACT.md` / temen `GC.md` §3.
...
- **`__vm_map` / `__vm_unmap` / `__vm_protect` / `__vm_page_size`** → the `Memory`
  ops. (Note: `temen-llvm`'s `malloc` already grows the window; the GC's own arena
...
- A C function calling each symbol lowers to the intended op (not a `call`),
  ...
  round-trip — the existing `temen-llvm` test convention.
