# Macro-staging migration: run compile-time macros on TEMEN

> **Status:** plan. Slice 2 of `TEMEN_SELFHOST_FEASIBILITY.md`. Goal: evaluate
> compile-time macros on the **TEMEN backend** instead of JACL's legacy bytecode VM
...
The TEMEN migration moved the **runtime** to TEMEN but left **compile-time** evaluation
on `vm.c`. So `jacl_compiler.temen` bundles the whole legacy interpreter and, at
compile time, runs macros on *(vm.c compiled to TEMEN)* — an interpreter on an
interpreter. This plan re-hosts macro evaluation on TEMEN so `vm.c`, `bytecode.c`, and
...
The blocker is that the **TEMEN runtime has no syntax support at all**:
...
- The value ABIs match in *format* — both are 64-bit tagged, same tag indices
  ...
  the legacy heap packs a **native pointer** in the payload (`JACL_PACK_PTR`), the TEMEN
  runtime packs a **window offset**. A legacy `JaclSyntax*` value is meaningless inside
  the TEMEN window. Heap objects are **not** bit-portable; they need reconstruction.
...
- `temen_codegen_program` (`codegen/codegen.c:4310`) is **whole-program, fixed-entry**:
...
- **Hygiene/gensym** (`vm.macro_scope_mark`, `vm.gensym_counter_ptr`) is host-side
  mutable state read *inside* the running macro; on TEMEN it must be passed in via ABI.

So re-hosting macros means porting JACL's syntax runtime + a macro-body codegen entry
+ cross-heap marshalling to the TEMEN backend. Substantial, but bounded and testable.
...
| TEMEN codegen (whole-program) | `IrModule *temen_codegen_program(AstNode**, u32, int module_mode, char*, size_t)` — `codegen/codegen.c:4310` |
...
through the current (`vm.c`) path yields golden IR; the TEMEN-staged path must produce
...
Each phase is independently landable and testable. `JACL_STAGE_ON_TEMEN` gates the new
...
- **Phase 1 — Syntax object model on `jaclrt`.** Add `JACL_TAG_SYNTAX` + `OBJ_SYNTAX`,
  ...
  Unit-test alloc + trace + field access in the TEMEN runtime. *Decision to make here:*
...
- **Phase 2 — Syntax builtins on TEMEN.** Re-express the `OP_SYNTAX_OP` surface
...
- **Phase 3 — Macro-body codegen entry.** A codegen path that compiles a single
  closure body to an TEMEN module whose entry is `f(syntax-args…) → syntax-result`
...
- **Phase 4 — Marshalling + hygiene ABI.** Reimplement `syntax_from_ast`/`syntax_to_ast`
  against the TEMEN heap (offset, not pointer); pass `macro_scope_mark` + a gensym
  counter into the module and read them from the TEMEN-side builtins; keep TEMEN-heap roots
  alive across the call.

- **Phase 5 — Staged pipeline.** Behind `JACL_STAGE_ON_TEMEN`, replace
  `jacl_ctx_run_closure` in `expand__node` with: compile the body (Phase 3) → run on an
  TEMEN runtime instance (Phase 1/2) → marshal args/result (Phase 4). Green when the whole
...
- **Phase 6 — Drop the legacy VM.** Remove the macro-path dependencies
  ...
  oracle (`compiler_compile`) and the `--interp` host round-trip — so `jacl_compiler.temen`
...
2. Re-implementing ~450 lines of syntax ops as TEMEN builtins (Phase 2).
...
5. `temen_codegen_program` is whole-program/fixed-entry — needs a macro-body entry (Phase 3).
...
`temen-llvm` only emits TEMEN imports for a **known capability / `__vm_*` family**
...
`temen-llvm/src/lib.rs:477`: "a call to one needs import support (a later slice)". So
**C-via-temen-llvm code cannot import another unit**; only **codegen** output emits arbitrary
...
- A new codegen entry `temen_codegen_staged_macro(names, lens, n, body)` emits a whole
...
- The Rust driver then reduces to `jacl_temen`'s exact path: link the codegen'd module
  against the (staging-extended) runtime, `synth_manifest_start`, run with the arg wire on
  stdin.

The `emit_macro_body` tool (codegen a body → `__jacl_macro` temen-text) is reusable as-is;
...
## Phase 3 final link — done; the arity-1 corpus runs end-to-end on TEMEN

The corrected design is built and **the whole pipeline compiles, links, runs on the TEMEN
...
- **Codegen entry** — `temen_codegen_staged_macro` (`codegen/codegen.c`): emits func 0 =
...
- **Driver** — `runtime/harness/src/bin/stage_macro.rs`: link vs the staging runtime,
  ...
  result wire on stdout. Reduces to `jacl_temen`'s link path.

**Verified on TEMEN** (`run_stage_test.sh` — the committed e2e gate): `k → 42`,
...
1. **Missing SelfData relocations.** String/data literals lower to data-segment references
   ...
   (`relocations:`), exactly like `jacl_temen`'s `split_relocs`.
...
`unless {cond body}`) stages on TEMEN. The design keeps the wire flat: the argument wire is a
...
codegen entry (`temen_codegen_staged_macro`) calls it once per parameter, in order, and binds
each. No args frame or per-arg func parameters — the TEMEN func signature is unchanged; the
parameters arrive over the wire. `run_stage_test.sh` covers the arity-2 case
`unless [== 1 2] { set hit 5 } → [if [== 1 2] {} { set hit 5 }]`.

## Phase 5 — the compiler stages macros on TEMEN (behind `JACL_STAGE_ON_TEMEN`) — done

The frontend's macro expander now runs macro bodies on the **TEMEN engine** instead of the
...
The seam is a hook, so `src/` gains no codegen/TEMEN dependency:

- **`src/syntax.c`** declares `jacl_macro_stage_hook` (NULL by default). In `expand__node`,
  when the hook is installed *and* `JACL_STAGE_ON_TEMEN` is set, a macro call is expanded through
...
- **`runtime/harness`** now also builds a `staticlib` exposing a C ABI, `jacl_temen_stage`
  ...
  embedding of "run on an TEMEN runtime instance."
- **`codegen/selfhost/macro_staging/stage_bridge.c`** is the hook body (codegen the body →
  `synw_encode` the argument ASTs → `jacl_temen_stage` → `synw_decode`), installed by the driver
  (`emit_jacl`, built with `-DJACL_STAGE_ON_TEMEN_BUILD`).
- **`run_diff.sh --temen`** is the committed gate: it builds that driver and diffs the
  `JACL_STAGE_ON_TEMEN=1` output against the same `golden/` the legacy path matches.
...
a jaclrt vec (the splice operand), and the staged entry (`temen_codegen_staged_macro`, now
taking a `variadic` flag) binds the fixed params via `synrt_read_arg` and the rest param via
`synrt_read_rest`. `run_diff.sh --temen` covers the corpus's `splice.jacl`
...
- **Computed binding names in the codegen backend.** `temen_codegen_program`'s binding forms
...
`run_diff.sh --temen` covers `corpus/gensym.jacl` (`fresh {v} = [mut [gensym] ~v]`) —
byte-identical to the oracle.

**Still deferred — scope-mark capture-hygiene.** Every legacy make-syntax op stamps
`macro_scope_mark`, yet the whole corpus (gensym included) emits identical IR with the TEMEN path
...
`bytecode.c`), so `jacl_compiler.temen` shrinks and no longer carries the legacy interpreter.
...
- **6.1 — cut the macro path's legacy dependency under staging (done).** When the TEMEN staging
  hook is active (`JACL_STAGE_ON_TEMEN`), `ast_expand_macros` now **skips** Phase 2 entirely — no
  bytecode closures are compiled; the hook codegens each body onto TEMEN per call. Verified: the
  whole corpus still emits byte-identical IR through `run_diff.sh --temen` with zero legacy
...
- **6.2 — make staging the default (done, minus the deletions — see below).** When the stage
  bridge is linked (the hook is installed), macros stage on TEMEN **by default**;
  `JACL_STAGE_ON_TEMEN=0` is the opt-out (used to regenerate the legacy oracle). A build without
...
- **6.3 — retire the standalone legacy-VM users that do *not* share the macro path** (partly
  ...
  **`--interp` deferred, not removable:** it is *not* dead — `parity.rs` spawns `emit_jacl
  ...
  "Moving it off `jacl_eval`" means **reimplementing `interpret` on TEMEN** (running guest JACL
...
- **6.4 — drop `vm.c` / `bytecode.c` / the bytecode half of `compiler.c` from `src/jacl.c`**,
  ...
  to stage (bridge-linked native, or guest-JIT in the `.temen`). Verify `jacl_compiler.temen`
  builds, shrinks, and the corpus still matches the frozen golden.

  Its two prerequisites — **guest-JIT macro staging** for the `.temen` and **`interpret` off
  the legacy VM** — are scoped in **`docs/TEMEN_GUEST_JIT_STAGING.md`**: the target TEMEN `Jit`
  capability (cap 11), the JACL-side gaps (a binary `temen-encode` serializer for `irbuilder`;
  a data-segment-free literal lowering; the staging runtime in the guest image + a
  `compile_linked` symtab), and the `interpret` isolation fork (host-cap-TEMEN-backed vs
