# TEMEN backend parity — bring-up notes

Working notes for grinding `docs/TEMEN_PARITY.md` (the generated scoreboard) toward
full corpus parity. This is the *how* and *why* behind the slices; the scoreboard
itself is the *what's-left*.

## Related references

- `docs/TEMEN_PARITY.md` — the generated scoreboard (what's left, bucketed by stage).
- `docs/TEMEN_RUNTIME_BUILTINS.md` — the codegen↔runtime `jacl_*` ABI, grouped by category.
- `docs/TEMEN_NUMERICS.md` — the numeric tower: inline vs boxed widths, overflow promotion,
  comparison/equality, error-flag propagation, print formatting.
- `docs/TEMEN_BUFFERS.md` — `[Buf N T]` / `[Ptr T]` model, zero-init, by-value params,
  buffer→pointer decay, and the fat-pointer arrow-access gaps.
- `docs/TEMEN_STREAMS.md` — generators-as-fibers, eager stream collection, and why the
  concurrency primitives score as hangs under the single-threaded oracle.
- `docs/TEMEN_FS_DESIGN.md` — the capability design for real file I/O (and named caps
  generally): guest-VFS fallback, `temen-fs` backends, data-image bundling, host policy.
...
frontend (lex → parse → typer)          # src/jacl.c, via codegen/tests/emit_jacl.c
  → TEMEN codegen (AST → TEMEN-IR)           # codegen/codegen.c
  → link against the translated runtime  # runtime/jaclrt.c, temen-llvm → temen_ir::link
  → powerbox + instantiate
  → run_diff (interp == jit)             # temen-run
...
`--release`: temen-jit's `gc.roots` scan trips a debug-only precondition otherwise).
...
and typed-closure conformance, and so on. Re-deriving all of those in the TEMEN codegen
...
- a passing program compiles cleanly → the TEMEN codegen runs and we score its output;
...
1. **Isolate the ASTs.** Both the compiler's conformance pass and the TEMEN codegen mutate
...
A handful of corpus cases will not be "fixed" because the TEMEN backend's behaviour is a
...
- **`overflow_call_depth` (scores `hang`).** The program is unbounded self-recursion in
  ...
  its `VM_FRAMES_MAX` (256) cap and surfaces `call depth exceeded`. The TEMEN codegen **does**
  ...
  nothing to gain and real behaviour to lose. We accept the divergence: the TEMEN backend is
...
- **Shell-out (`io_pipe_to_file`, part of `tour`).** `!cmd` / `| write-file` need a host
  process-exec capability. The TEMEN powerbox grants stdout/stdin/exit only, by design — there
  is no subprocess cap to spawn `echo`. Blocked on a sandbox-capability decision, not codegen;
  the upstream ask (an `exec` cap in the temen-fs mold, with a deterministic scripted backend)
  is written up in `docs/TEMEN_EXEC_ASK.md`.
...
- **Buffers + pointers — DONE.** Scalar-leaf `[Buf N T]` (any nesting depth) is now a
  ...
  raw bytes, and C externs dereference decayed buffer pointers. See `TEMEN_BUFFERS.md`.
...
- **File I/O (`read-file`/`write-file`) — passing, via the guest VFS.** The runtime
  ...
  a parity gap — see `TEMEN_FS_DESIGN.md` for the layered plan (guest VFS fallback ·
...
- **Typed-collection printing.** The old VM prints a typed `[Vec T]` as `[1, 2, 3]` and an
  ...
  (`__vm_gc_roots`, an opaque temen op) can't be verified to mask an unfamiliar tag byte off a
...
| file I/O (`read/write-file`) | 0 (passing) | infra | guest VFS serves; real fs is the capability design in `TEMEN_FS_DESIGN.md` |
