# Ask for temen — a frontend-independent powerbox instantiation layer

**Context.** temen already has the wasm-like pieces: named capability **imports**
(`resolve_imports(module, resolver)` → `cap.call`), named **exports** + cross-module
binding (`temen_ir::link`), and host grants (`Host::grant_stream`/`grant_host_fn`). temen-llvm
...
region) — is **baked into temen-llvm's C→IR translation** (`synth_start`). A frontend that
emits TEMEN-IR directly and links it itself (e.g. JACL's codegen) can't reach it, and
...
**Acceptance.** A **hand-written IR module** (no C, no temen-llvm) that imports `write` and
