# TEMEN runtime builtins — the codegen↔runtime ABI

Every JACL head the TEMEN codegen (`codegen/codegen.c`) can't lower inline becomes a
`call.import` to a `jacl_*` C entry point in `runtime/` (translated to TEMEN IR by
`temen-llvm` and linked into the program module). The uniform ABI is:
...
next contributor picking a head off the `docs/TEMEN_PARITY.md` worklist — not exhaustive
runtime internals.
...
`[Buf N T]` is an array (see `docs/TEMEN_BUFFERS.md`). `addr $buf->i` → `jacl_addr_of`
