# Running the JACL compiler *on* TEMEN — feasibility (measured)
...
> **Emscripten**). Could we instead compile the JACL **compiler itself** to TEMEN IR
> (via the `vendor/temen` LLVM on-ramp) and run it as an TEMEN guest on
> `temen_browser.wasm` — one engine, no emcc? **Yes.** This note records the numbers.
>
> Tooling: `clang-18` + `llvm-link-18` (C → LLVM IR), `vendor/temen`'s
> `temen-llvm-translate` / `try_translate` (LLVM IR → TEMEN IR → verify → run). The
> reproducible harness lives in `codegen/selfhost/`.

## TL;DR

The whole JACL frontend compiles to TEMEN IR, verifies, runs on the TEMEN engine, and
**compiles JACL end-to-end as an TEMEN guest** — source in, correct TEMEN IR out.

| Question | Measured result |
|---|---|
| temen-llvm translates the whole compiler? | **Yes** — 1027 funcs (frontend lib) / 1031 (with driver), verified |
| Runs on the TEMEN engine? | **Yes** — executes, clean exit |
| Compiles JACL end-to-end on TEMEN? | **Yes** — `[+ 1 [* 2 3]]` → correct tagged-`7` TEMEN IR |
...
TEMEN on-ramp makes the **entire JACL compiler a portable TEMEN module** that runs
anywhere TEMEN runs, unifies the playground on one engine, and dogfoods the on-ramp.
It also composes with the generic `temen_link_run` we already landed: the
compiler-guest emits IR → `temen_link_run` links it against `jaclrt.temen` and runs it.
`emcc` was the cheaper way to *ship the playground*; this is the better architecture
if "TEMEN everywhere" is a goal.
...
TEMEN_STUB_EXTERNS=1 try_translate jacl_frontend.ll
```
...
escape-freedom **verifier**, and **executed** on the TEMEN engine — the built-in CLI
...
### 4. End-to-end: JACL source → TEMEN IR, on the TEMEN engine
...
The compiler-guest produced correct TEMEN IR for `1 + 2*3 = 7` (tagged small-int).
...
`vendor/temen/crates/temen-run/demos/postgres/printf_shim.c` + string shims instead of a
hand-rolled one.

## Caveats

1. **On-ramp version.** Measured against the current `vendor/temen` checkout. A newer
   temen has grown the wasm-JIT subset (e.g. chibicc moved to `jit:true`, i.e. float
...
1. **Self-host build harness** (`codegen/selfhost/`) — a reproducible
   `build_compiler_temen.sh` (emit TUs + driver + shim → `llvm-link` →
   `temen-llvm-translate` → `jacl_compiler.temen`), the stdin→IR driver
   (`emit_driver.c`), and the emit-path shim (`emit_shim.c`). Guards on
   `clang-18`/temen-llvm; skips cleanly when absent (like the temen Tcl demo). This
...
2. **Macro-staging migration** — the substantive one. Compile-time macro
   ...
   in — it is *not* dead. Post-TEMEN, macros should be **staged on TEMEN**: compile the
   macro body through the same `codegen/` path and execute it on the TEMEN engine at
   compile time, so `vm.c` + the bytecode half of `compiler.c` drop out and the
   toolchain has **one backend**. Full multiphase plan:
   [`TEMEN_MACRO_STAGING_PLAN.md`](TEMEN_MACRO_STAGING_PLAN.md).
...
4. **Browser wiring** — ship `jacl_compiler.temen`; run it on `temen_browser.wasm`,
   feeding edited source via stdin and capturing emitted IR via stdout, then hand
   that IR to the existing `temen_link_run`. Behind a flag; keep `jacl_emit.wasm`
   (emcc) as the fallback until parity is proven.
5. **Tier selection** — measure bytecode vs wasm-JIT on current temen; adopt tier-up
   if the module emits.
...
See `codegen/selfhost/README.md`. The build script produces `jacl_compiler.temen`;
feed a JACL program on stdin to get TEMEN IR on stdout.
