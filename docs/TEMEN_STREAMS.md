# Streams, generators, and concurrency on the TEMEN backend
...
the TEMEN codegen mirrors that via `Cx.cur_is_generator`.

## Streams collect eagerly

The corpus builds stream pipelines — `collect`, `transform`, `filter`, `take` — that are
lazy in the old VM. The TEMEN backend materialises them **eagerly**: `collect` drains its
...
discussion in `docs/TEMEN_PARITY_NOTES.md`.
...
1). This is the configuration correct on BOTH temen backends — and the reference
