...
independent wins temen now supports:

1. **Warm-runtime snapshot** — run the compiler's program-independent init **once** at page load,
   snapshot it, and restore it per compile (temen's `temen_warm_open`/`_eval`/`_close`). Removes the
   fixed per-compile init cost.
2. **wasm-JIT tier** — a page-op-free guest is JIT-eligible, so the compiler's hot functions run as
   emitted wasm instead of interpreted bytecode (temen's slice-2 default-on / opt-in tier-up). Removes
...
to the interpreter (temen #750), and (b) the snapshot restore is a window memcpy that can't reproduce a
grown region's `Mem` page-state (temen #816). Pre-size the window big enough that the heap never grows
→ both problems vanish. temen's own warm-snapshot prototype does exactly this (`WARM_MAPPED_LOG2` =
...
- **This is a self-hosting win, not a live perf need.** The playground is already fast via the AOT
  ...
  here is *architectural*: one compiler, compiled entirely through TEMEN, no separate Emscripten
...
- **Ceiling.** Even done, the JIT'd guest lands *below* `jacl_emit` (the wasm-JIT carries confinement
  masking on every access that Emscripten's native-linear-memory build does not; temen slice-3 elision
...
## What temen provides (all landed as of pin `63a70532`)

- `temen_warm_open(mod_ptr, mod_len) -> i64` — decode a two-phase driver module, enlarge the mapped
  window to `WARM_MAPPED_LOG2`, run its `warmup` export **once**, keep the live prefix `[0, brk)` as
  the warm image. (`browser/src/lib.rs`.)
- `temen_warm_eval(stdin_ptr, stdin_len) -> i64` — restore the warm image into a fresh window, run
  `eval_run` with stdin seeded, stage stdout into the same capture slots `temen_run_onramp` uses.
- `temen_warm_close()` — free the session.
- Slice-2 default-on JIT + the "warm+JIT" compose (`2b0f65bf`): the warm snapshot's `eval_run` can
  itself run on emitted wasm. Reference driver: `crates/temen-run/demos/quickjs/qjs_snapshot.c`
  (`main`/`warmup`/`eval_run`); native test `crates/temen-llvm/examples/qjs_snapshot.rs`; browser test
...
- **Heap** — `runtime/heap_gc.c:27`: the GC heap is **already a fixed static region** (no window
  ...
  the frontend's non-GC allocations), which is temen #737's 4 KiB-`vm_map` area.

## Slices

### Slice 0 — measure the floor — DONE. Verdict: **JIT-tier first, warm-snapshot deferred.**

Measured `jacl_compiler.temen` compiling several programs through `temen_run_onramp` (bytecode
interpreter, the live playground path), median of 25 runs, via `demo/temen/bench_guest.mjs`:
...
- Therefore: **do Slice 1 → measure JIT (promoted below) first.** Defer the warm-snapshot frontend
  ...
  decoded/compiled-module cache across compiles (temen's slice-1 browser cache is the JIT-path
...
the interpreter via `env.call_interp` — so it sits closer to temen's Lua/SQLite JIT regime (~1.3–3×)
...
path is **per-function mainline tier-up over the live window** (exactly what temen slice-2 just
...
### Slice 0b — tier-up eligibility measured against temen's pump (#839) — CONFIRMS the blocker

temen has since wired the single-shot **tier-up pump** into the on-ramp cards (#809/#831/#888): the
cdylib now exposes `temen_onramp_tierup_open` + `driveTierupRun` (the playground's `runJitModule` falls
through to it for an `InterpDriven` `_start`). #839 asks to bench the JACL card through it. Measured
(`demo/temen/bench_tierup.mjs`, `temen_browser.wasm` @ `d6f34a49`):

- **The pump refuses the JACL compiler-guest outright** — `temen_onramp_tierup_open` returns
...
- **Why (from `temen_onramp_tierup_open`'s own gate):** it admits fibers and `vm_jit_*` guests, and
...
`demo/temen/bench_tierup.mjs`, threads `temen_browser.wasm` @ `85d8cf5d`):
...
After the `temen→temen` migration merged (jacl_impl PR #73), the shipped `temen_browser.wasm` was found
...
`--shared-memory --import-memory`, `-Z build-std`) and re-ran `demo/temen/bench_tierup.mjs`. Three
...
**Measured (`demo/temen/bench_tierup.mjs`, threads `temen_browser.wasm` @ `85d8cf5d`):**
...
- **Pin bumped** to temen `fdeb72a9` (main, post-#1319). The migration this required on the JACL side:
  ...
  - harness `probe_temen`/`browser_coverage` for `CapCall.sig` (#922 type index) + `compile_module`'s
...
- **Re-measured (`demo/temen/bench_tierup.mjs`, threads `temen_browser.wasm` @ `fdeb72a9`):** the
...
- **Native test — `SNAPTEST` in `build_compiler_temen.sh --selftest`:** `warmup()` once, then compile
...
- `build_compiler_temen.sh` builds **`jacl_compiler_snapshot.temen`**; `build_assets.sh` ships it under
  `temen/cards/` (best-effort, alongside `jacl_compiler.temen`).
- `TemenJaclRunner` (`demo/src/temen-jacl-wasm.ts`): `warmOpen(snapshotTemen)` / `warmEval(source)` /
...
- `playground.ts`: `ensureLive` loads the warm card; the **`tierup` mode** (previously a placeholder
  ...
  bundle both clean.
- **End-to-end measured** (`demo/temen/warm_probe.mjs`, threads `temen_browser.wasm` @ `85d8cf5d`,
...
- **Per-function mainline tier-up over a live growing window** — the original spike
  (`TEMEN_BROWSER_TIERUP_FINDINGS.md`); superseded here by "pre-size the window so it never grows,"
