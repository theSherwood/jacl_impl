...
/* buf==NULL means "using inline_buf". This avoids a dangling self-referential
   pointer when the struct is returned/copied by value. */
static inline uint8_t* rdoc_builder_buf(rdoc_builder* b) {
  return b->buf ? b->buf : b->inline_buf;
}

static inline rdoc_builder rdoc_builder_new(void) {
  rdoc_builder b;
  b.buf   = NULL;  /* NULL = using inline_buf */
  b.len   = 0;
...
static inline bool rdoc_builder_ensure(rdoc_builder* b, size_t extra) {
  ...
  memcpy(new_buf, rdoc_builder_buf(b), b->len);
  if (b->buf) {
    rdoc_st_allocator.free(rdoc_st_allocator.ctx, b->buf);
  }
  b->buf = new_buf;
...
static inline bool rdoc_builder_text(rdoc_builder* b, const uint8_t* str, size_t byte_len) {
  ...
  memcpy(rdoc_builder_buf(b) + b->len, str, byte_len);
  b->len += byte_len;
  return true;
}
...
static inline rdoc rdoc_builder_finish(rdoc_builder* b) {
  ...
  rdoc d = rdoc_from_bytes(rdoc_builder_buf(b), b->len);
