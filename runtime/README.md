# JACL runtime (on TEMEN) — Phase 1

The JACL runtime, written in C and compiled to TEMEN IR via `temen-llvm`. Scope and
step plan: `docs/TEMEN_BACKEND_PHASE1.md`. Architecture: `docs/TEMEN_BACKEND_DESIGN.md`.
...
- `harness/` — Rust test harness: compiles a driver (+ runtime) with
  `clang -O2 -emit-llvm`, translates via `temen-llvm`, verifies, and runs `run` on
...
separate runtime-artifact linking (which needs an temen-llvm name→index export map)
is a Phase-2 prerequisite, proven independently by `spikes/temen_emit_link`.
