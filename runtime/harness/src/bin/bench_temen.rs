//! bench_temen — execution-time benchmark for the TEMEN backend.
//!
...
//! real TEMEN pipeline and times **execution only**, hoisting the one-time costs
//! (temen-llvm runtime translation, codegen driver build, link, JIT compile) out of
...
/// Build the codegen driver (same recipe as parity.rs / jacl_temen.rs).
fn build_driver() -> PathBuf {
    let out = std::env::temp_dir().join(format!("bench_temen_emit_{}", std::process::id()));
...
fn main() {
    ...
    if files.is_empty() {
        eprintln!("usage: bench_temen <scenario.jacl>...");
        std::process::exit(2);
    }
    ...
    eprintln!("bench_temen: building codegen driver…");
    let driver = build_driver();
    eprintln!("bench_temen: translating runtime (temen-llvm)…");
    let rt = jacl_runtime_harness::translate_runtime();
    let cat = jacl_runtime_harness::translate_catalog();
    eprintln!(
        "bench_temen: runtime translated ({} funcs). iters={iters} warmup={warmup} interp_iters={} interp={}",
        rt.module.funcs.len(),
        interp_iters,
        !no_interp,
    );
    ...
    match std::env::var("BENCH_JSON_OUT") {
        Ok(path) => {
            ...
            eprintln!("\nbench_temen: wrote {} scenarios to {path}", out_lines.len());
        }
        Err(_) => println!("{json}"),
    }
}
