...
//! (the browser's only wasm-safe engine), exactly as the `temen-browser` cdylib's `temen_run_onramp`
...
//!        Writes docs/TEMEN_BROWSER_COVERAGE.md unless --no-doc.
...
/// `exit`→exit) — mirroring `temen-browser`'s `grant_onramp_caps` / `onramp_cap_resolver`.
...
fn run_case(driver: &PathBuf, rt: &temen_llvm::Translated, cat: &temen_llvm::Translated, case: &Case, fuel_cap: u64) -> Stage {
    ...
    if bytecode::compile_module(&module.funcs, &module.types, None).is_none() {
...
fn main() {
    ...
    if write_doc {
        ...
        let _ = writeln!(md, "# TEMEN browser coverage — corpus on the wasm-safe bytecode engine\n");
        ...
        let _ = writeln!(md, "> name-bound powerbox — the exact path the `temen-browser` cdylib's `temen_run_onramp`");
        ...
        let path = format!("{ROOT}/docs/TEMEN_BROWSER_COVERAGE.md");
