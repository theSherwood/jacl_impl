//! emit_temen — spike tool (docs/TEMEN_BROWSER_PLAN.md).
//!
...
//! (instead of running it, as `jacl_temen run` does). The image is what the browser
//! `temen-browser` cdylib's `temen_run_pb` decodes and runs.
//!
//!   emit_temen <prog.jacl> <out.temen>
//!
//! The pipeline mirrors `jacl_temen.rs` exactly up to `synth_manifest_start`.
...
fn build_driver() -> PathBuf {
    let out = std::env::temp_dir().join(format!("emit_temen_driver_{}", std::process::id()));
...
fn main() {
    ...
    // Two input shapes:
    //   emit_temen <prog.jacl> <out.temen>   — compile via the emit-only C driver (no macros), or
    //   emit_temen --ir <prog.ir> <out.temen> — link + encode TEMEN-IR produced elsewhere. The `--ir`
    ...
    let (program, out_path) = match args.as_slice() {
        ...
        _ => {
            eprintln!("usage: emit_temen <prog.jacl> <out.temen>  |  emit_temen --ir <prog.ir> <out.temen>");
            std::process::exit(2);
        }
    };

    eprintln!("emit_temen: translating runtime + catalog…");
    ...
    eprintln!("emit_temen: wrote {} ({} bytes)", out_path.display(), bytes.len());
}
