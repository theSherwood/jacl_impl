//! jacl-temen — the TEMEN-backend CLI (phase 3 of `docs/TEMEN_FS_DESIGN.md`).
//!
...
//! ```
//!
//! Grants are **explicit and default-deny** (TEMEN_FS_DESIGN.md, design rule 1): with no
...
//!
//! The pipeline is the parity harness's: frontend+codegen emit driver → temen-ir link
...
fn build_driver() -> PathBuf {
    let out = std::env::temp_dir().join(format!("jacl_temen_emit_{}", std::process::id()));
...
fn die(msg: &str) -> ! {
    eprintln!("jacl-temen: {msg}");
    std::process::exit(1);
}

/// `bundle <assets-dir> -o <out.fsimg>` — walk a host directory into an fs seed and
/// serialize it as a data image (`temen-fs` `read_host_dir` + `encode_image`). The image is
...
fn cmd_bundle(args: &[String]) {
    ...
    eprintln!(
        "jacl-temen: bundled {} file(s), {} dir(s), {} content byte(s) -> {} ({} bytes)",
        files.len(),
        dirs.len(),
        bytes,
        out.display(),
...
fn cmd_run(args: &[String]) -> ! {
    ...
    // Compile: frontend + TEMEN codegen via the emit driver.
    ...
    eprintln!("jacl-temen: translating runtime…");
    ...
    // docs/TEMEN_IMPORT_MIGRATION_ASK.md), wrap the program entry (its func 0) in the paramless
    ...
    // (docs/TEMEN_BROWSER_SPIKE_FINDINGS.md) declines the linked runtime — so output matches
