/* The Unir edge behind a C ABI (issue #18 spike). A C program links these by name against
 * the `unir-cabi` temen unit (temen_ir::link). Every call takes the vat it acts in.
 *
...
/* The root vat. Regions map in [map_base, map_end); children are carved at
 * carve_base + i << carve_log2 for i < carves. All of it must be free window space. */
unir_vat *unir_vat_root(uint64_t map_base, uint64_t map_end, uint64_t carve_base,
                        uint32_t carve_log2, uint32_t carves);

/* A fresh zeroed region of `len` bytes; returns its capability. */
int64_t unir_region_create(unir_vat *vat, uint64_t len);

/* Spawns the granted child image with private argument bytes, endowed with `cap` under
 * `name` and `fuel`; returns the child's capability. */
int64_t unir_spawn(unir_vat *vat, const uint8_t *args, uint64_t args_len, const char *name,
                   uint64_t name_len, int64_t cap, uint64_t fuel);
