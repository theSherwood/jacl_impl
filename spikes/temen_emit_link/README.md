# Spike-1 — JACL → TEMEN emit / verify / static-link / run

**Status: PASS** (temen `vendor/temen` @ f75509a).

Retires the riskiest unknowns of the TEMEN backend's *front door* before any bulk
work (`docs/TEMEN_BACKEND_PLAN.md` §2):

1. A JACL-shaped program emitted as **TEMEN text IR** parses and passes the
...
3. `temen_ir::link` **static-links** a separately-authored *runtime* unit against the
   *program* units, resolving a cross-module call (`rt_add`) by name.

## Run

```
cd spikes/temen_emit_link && cargo run
```
...
It builds the temen crates from the submodule (Cranelift JIT included), so the first
run takes ~45s.
...
- The "runtime" unit (`rt_add`) stands in for JACL's LLVM-compiled runtime; here it
  ...
  `docs/TEMEN_LLVM_INTRINSICS_ASK.md`). The `emit_*` functions stand in for JACL's
  codegen emitter (in the real backend, the C emitter).

## Finding that shapes JACL's codegen

**temen uses block-local SSA.** A block may reference only its own block parameters
