//! Spike-1 — JACL → TEMEN emit/verify/link/run path.
//!
//! Goal: retire the riskiest unknowns of the TEMEN backend's *front door* before any
//! bulk work:
//!   1. Can a JACL-shaped program be emitted as TEMEN **text IR**, parse, and pass
...
//!   3. Does `temen_ir::link` **static-link** a separately-authored *runtime* unit
...
use temen_ir::{link, LinkUnit, Module};
...
/// (const 0) before the arg tuple, per temen's text form; the linker rewrites the
...
///
/// temen uses **block-local SSA**: a block sees only its own params + local defs, so
...
fn unit(src: &str, exports: &[(&str, u32)]) -> LinkUnit {
    let module = match temen_text::parse_module(src) {
        Ok(m) => m,
        Err(e) => {
...
fn main() {
    println!("Spike-1: JACL → TEMEN emit / verify / static-link / run\n");
