...
**Status: PASS** (temen `vendor/temen` @ f75509a; needs `clang`, `cc`, `llvm-18-dev`).

Validates the headline simplification of the backend rewrite (`docs/TEMEN_BACKEND_DESIGN.md`
§4.3.1): everything JACL's compile-time **state-machine transform** does for
generators, temen **stackful fibers** (`cont.*`, exposed in C as `__vm_fiber_*`) do at
...
```
cd spikes/temen_fiber_generator && cargo run
```

```
[2] temen-llvm translated it (6 funcs); imports: []
[3] verifier accepted the translated module
[4] run (interp == jit), vs closed form a+b+c:
  PASS  run(5) = 85
  PASS  run(10) = 520
  PASS  run(20) = 3440
[5] native oracle (no fibers): run(5) = 85  (matches temen)
```

## What it demonstrates

A generator runtime written in plain C on the fiber primitives, compiled
`clang -O2 -emit-llvm` → `temen_llvm` → run on interp + Cranelift JIT:
