...
//! generators (resumable producers of a value sequence), temen **stackful fibers**
...
//!     M:N scheduler needs).
//! Compiled with `clang -O2 -emit-llvm`, translated by `temen_llvm`, run on interp +
...
fn native_run5() -> i32 {
    let dir = std::env::temp_dir();
    // temen builtins are undefined natively; compile a native shim that defines
...
fn main() {
    ...
    println!("[5] native oracle (no fibers): run(5) = {native}  (matches temen)");
