...
**Status: PASS** (temen `vendor/temen` @ f75509a; needs `clang` + `llvm-18-dev`).

The riskiest design point of the backend rewrite (`docs/TEMEN_GC_CONTRACT.md`,
`docs/TEMEN_BACKEND_PLAN.md` §2): prove a guest-side **conservative, non-moving
mark-sweep** collector — finding roots via temen's `gc.roots` op — reclaims garbage
and retains the reachable graph (with intact data) across collections.

The collector is plain C compiled through temen-llvm, so it runs the **real
`__vm_gc_roots` op** on interp + Cranelift JIT.

## Run

```
cd spikes/temen_gc && cargo run
```

```
[2] temen-llvm translated it (7 funcs); imports: []
...
- **`gc.roots` found the live stack root through the call chain** (`run` →
  ...
  bite this scenario.
- **temen-llvm rejects a *constant* `ptrtoint` of a global address** (folding
  ...
  codegen note for JACL's runtime (and a candidate temen-llvm enhancement).
