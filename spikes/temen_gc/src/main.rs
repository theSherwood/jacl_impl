...
//!
//! The riskiest design point of the backend rewrite (`docs/TEMEN_BACKEND_PLAN.md` §2,
//! `docs/TEMEN_GC_CONTRACT.md`): prove that a guest-side **conservative, non-moving
//! mark-sweep** collector, finding its roots via temen's `gc.roots` op, **reclaims
...
/* Runtime ptr->int (noinline) — temen-llvm rejects a *constant* ptrtoint of a global
...
fn main() {
    ...
    let module = t.module;
    println!("[2] temen-llvm translated it ({} funcs); imports: {:?}",
