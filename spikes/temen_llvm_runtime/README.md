# Spike-1b — JACL runtime C through the temen-llvm on-ramp

**Status: PASS** (temen `vendor/temen` @ f75509a; needs `clang`, `cc`, and **`llvm-18-dev`**
installed for temen-llvm's libLLVM linkage).

Validates the single biggest effort assumption of the backend rewrite
(`docs/TEMEN_BACKEND_PLAN.md` §8): that JACL's existing runtime C can be **recompiled**
through temen's LLVM on-ramp rather than re-authored.
...
with `temen_llvm::translate_bc_path`, verifies, and runs `run(n) = sum(0..n-1)` on
**interp + Cranelift JIT**, cross-checked against the closed form and native `cc`:

```
[2] temen-llvm translated it to TEMEN IR (6 funcs)
    imports: []
...
[5] native cc oracle: run(16) exit code = 120  (matches temen)
```
...
temen-llvm's own `vm_memory_map_and_page_size` test.)

## (b) The concurrency/GC intrinsic surface works

The `__vm_*` ops JACL's runtime needs are exercised by temen-llvm's own tests
(run from `vendor/temen/crates/temen-llvm`):
...
```
# one-time, for temen-llvm's libLLVM linkage:
apt-get install -y llvm-18-dev      # matches the container's clang/llvm 18
cd spikes/temen_llvm_runtime && cargo run
```
