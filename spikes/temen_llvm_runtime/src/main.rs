//! Spike-1b(a) — compile a REAL JACL collection through `temen-llvm` and run it.
//!
//! Thesis under test: JACL's existing runtime C (the pointer-chasing, struct-heavy,
//! malloc-using systems code in `lib/`) can be **recompiled** through temen's LLVM
//! on-ramp rather than re-authored — the single biggest effort assumption of the
//! backend rewrite (`docs/TEMEN_BACKEND_PLAN.md` §8).
//!
...
//! with `temen_llvm::translate_bc_path`, verify, and run on **interp + Cranelift JIT**,
...
#include <stdint.h>
#include <stdbool.h>
#include <string.h>   /* memcpy/memset — temen-llvm synthesizes */
...
/* Self-contained bump arena over a static buffer — keeps the spike focused on the
   ...
   resolve; window growth is separately proven by temen-llvm's own memory tests). */
...
fn main() {
    println!("Spike-1b(a): real JACL lib/rrb_vec/rrb_vec.h through temen-llvm\n");
    ...
    // 2. translate the bitcode to TEMEN IR via the on-ramp ----------------------
    let t = temen_llvm::translate_bc_path(&bc).expect("temen-llvm: translate bitcode");
    let module = t.module;
    println!("[2] temen-llvm translated it to TEMEN IR ({} funcs)", module.funcs.len());
    ...
    println!("[5] native cc oracle: run(16) exit code = {native}  (matches temen)");

    println!("\nSpike-1b(a) OK — JACL's real RRB persistent vector compiles through \
              temen-llvm and runs correctly on interp and JIT, matching native cc.");
}
