...
**Status: root-caused; not shipped; no temen dependency.** This spike chased a flaky GC corruption
...
continuation scheduler riding on temen's **already-implemented, already-tested** cross-vCPU fiber
migration — so the persistent M:N pool is **not** blocked on temen.

It corrects two earlier wrong conclusions: temen's `gc.roots` is **not** buggy, and fiber
...
only on **main's fiber stack/registers**. temen's `gc.roots` does not reliably cover the main
...
temen-llvm + the JIT's real OS threads the plain store was not ordered before the atomic, so an
...
it, with `gc.roots` covering migrated-but-suspended fibers) — which **temen already implements and
tests** on both backends (`cont.resume` "any vCPU may resume … even when another OS thread
suspended it"; `temen/tests/fiber_migrate.rs::fiber_suspended_on_root_resumes_on_spawned_vcpu`,
`gc_roots.rs` cross-vCPU scan). So #3 is **not** blocked on temen — it is remaining JACL-side
work. `docs/TEMEN_PHASE3_ASKS.md` Ask 2 (which earlier requested migration) is **withdrawn**.

## Reproduce

```
cp spikes/temen_pool_gc/sched_pool.c        runtime/sched.c
cp spikes/temen_pool_gc/test_parallel_gc.c  runtime/tests/test_parallel_gc.c
...
(Use `--release`: temen-jit's `gc.roots` stack scan trips Rust's debug-only `read_unaligned` UB
precondition check — a separate, pre-existing temen debug assertion, benign in release.)
