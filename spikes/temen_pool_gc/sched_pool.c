 ...
 * per construct): temen vCPU ids are dense and never reused, so a program with many
 ...
 * fiber chain on a *spawned* vCPU cannot quiesce temen's stop-the-world gc.roots scan cleanly;
 ...
 * DONE store under temen-llvm + the JIT's real threads (the awaiter could see DONE with a stale
