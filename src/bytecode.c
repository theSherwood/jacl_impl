...
typedef enum {
  ...
  OP_SLEEP_SM,       /* SM sleep: pop seconds (number); register timer with runtime, suspend SM (return VM_OK) */
  OP_SLEEP_BLOCK,    /* Blocking sleep: pop seconds (number); nanosleep blocking on current thread, push nil — used in non-SM contexts */
  ...
  OP_BUF_GET_STRUCT_LOCAL,  /* u8 base_slot, u16 struct_idx, u16 buf_len;
                               pop i32 index, bounds-check, push N inline
...
const char* bytecode__opcode_name(uint8_t op) {
  switch ((OpCode)op) {
    ...
    case OP_SLEEP_SM:            return "OP_SLEEP_SM";
    case OP_SLEEP_BLOCK:         return "OP_SLEEP_BLOCK";
