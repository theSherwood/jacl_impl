...
/* ---- Recursive layout descriptor (RECURSIVE_LAYOUT_REFACTOR.md) ----
 *
 ...
 * compositional -- a buf tiles its element's map N times; a struct unions its
 * fields' maps at their offsets -- so it is correct by construction for
 * arbitrary nesting, which is what eliminates the nested-buf / struct-leaf gap
 * class. Step 1 builds it and self-checks it against the existing per-struct
 * slot_ref_bitmap; later steps route GC + literal-init through it. */
static uint32_t compiler__encoding_align(StructTypeRegistry* reg,
                                         uint32_t encoded) {
  if (JACL_IS_SCALAR_TYPE_IDX(encoded)) {
    return struct__type_align(JACL_TYPE_IDX_TO_SCALAR(encoded), NULL, 0);
  }
  if (!reg || encoded >= reg->count) return 1;
  TypeShape* s = &reg->shapes[encoded];
  switch (s->kind) {
    case TYPE_SHAPE_STRUCT:
    case TYPE_SHAPE_CTX:
      return s->u.struct_def ? s->u.struct_def->alignment : 1;
    case TYPE_SHAPE_BUF:
      return compiler__encoding_align(reg, s->u.buf.elem_idx);
    case TYPE_SHAPE_TYPED_VEC:
    case TYPE_SHAPE_TYPED_MAP:
    case TYPE_SHAPE_PTR:
    case TYPE_SHAPE_FUTURE:
...
/* OR the GC-traced-slot map for `encoded` into `bitmap`, with slots numbered
 * from `byte_base` (each slot is sizeof(JaclVal) bytes). `bitmap_slots` caps
 * the writable range. Ref types are 8-byte sized + aligned, so they always
 * land on a whole slot boundary; value scalars never mark. */
static void compiler__encoding_ref_map(StructTypeRegistry* reg,
                                        uint32_t encoded,
                                        uint32_t byte_base,
                                        uint8_t* bitmap,
                                        uint32_t bitmap_slots) {
  if (JACL_IS_SCALAR_TYPE_IDX(encoded)) {
    JaclType t = JACL_TYPE_IDX_TO_SCALAR(encoded);
    bool is_ref = (t == TYPE_DYN || t == TYPE_STR || t == TYPE_VEC ||
                   t == TYPE_MAP || t == TYPE_CLOSURE || t == TYPE_STREAM);
    if (is_ref) {
  ...
  if (!reg || encoded >= reg->count) return;
  TypeShape* s = &reg->shapes[encoded];
  switch (s->kind) {
    case TYPE_SHAPE_TYPED_VEC:
    case TYPE_SHAPE_TYPED_MAP:
    ...
    case TYPE_SHAPE_BUF: {
      uint32_t elem_enc = s->u.buf.elem_idx;
      uint64_t elem_sz = compiler__encoding_byte_size(reg, elem_enc);
      for (uint32_t i = 0; i < s->u.buf.len; i++) {
        compiler__encoding_ref_map(reg, elem_enc,
            byte_base + (uint32_t)(i * elem_sz), bitmap, bitmap_slots);
      }
      return;
    }
    case TYPE_SHAPE_STRUCT:
    case TYPE_SHAPE_CTX: {
      StructTypeDef* sd = s->u.struct_def;
      if (!sd) return;
      for (uint32_t fi = 0; fi < sd->field_count; fi++) {
        StructTypeField* f = &sd->fields[fi];
        uint32_t foff = byte_base + f->offset;
        if (f->type == TYPE_BUF) {
          /* Buf field: element encoding in struct_type_idx, N in buf_len. */
          uint32_t elem_enc = f->struct_type_idx;
          uint64_t elem_sz = compiler__encoding_byte_size(reg, elem_enc);
          for (uint32_t i = 0; i < f->buf_len; i++) {
            compiler__encoding_ref_map(reg, elem_enc,
                foff + (uint32_t)(i * elem_sz), bitmap, bitmap_slots);
          }
        } else if (f->type == TYPE_STRUCT) {
          compiler__encoding_ref_map(reg, f->struct_type_idx, foff,
                                     bitmap, bitmap_slots);
        } else {
          /* Scalar field. Struct ref-scalar fields are rejected at decl,
           * but recurse generically so the map stays a faithful descriptor. */
          compiler__encoding_ref_map(reg, JACL_SCALAR_TYPE_IDX(f->type), foff,
                                     bitmap, bitmap_slots);
        }
      }
      return;
...
static bool compiler__try_compile_nested_buf_chain(Compiler* c,
  ...
  /* Compile-time bounds check for LIT_INT indices. Dynamic indices
   * are bounds-checked at runtime by OP_PTR_OFFSET_CHECKED below;
   * the per-dim size (dims[k]) is baked into the opcode operand. */
  ...
  for (int k = dyn_start; k <= chain_depth; k++) {
    ...
    /* Bounds-check dynamic indices at runtime. Literal indices were
     * already validated at compile time and folded into prefix_offset
     * or pending above. */
    compiler__emit_byte(c, OP_PTR_OFFSET_CHECKED, line);
    compiler__emit_u16(c, (uint16_t)strides[k], line);
    compiler__emit_u16(c, (uint16_t)dims[k], line);
  }
...
void compiler__compile_hof_builtin(Compiler* c, const char* name,
  ...
  } else if (opcode == OP_FILTER || opcode == OP_EACH) {
    /* Typed-closure callback (TYPED_CLOSURES_DESIGN.md Phase A): bake the rep
     * the predicate/each-callback param expects. For an inline callback over a
     * typed stream with a wide (i64/u64/f64) element, the typer typed its body
     * to read the element wide (HEAD_FILTER / HEAD_FOR) and stamped
     * args[1]->inferred_struct_idx with the wide element enc iff it
     * monomorphized cleanly (else UINT32_MAX). Trust that verdict so the baked
     * rep matches how the body was typed: wide enc → the pull passes the
     * element wide with no box; 0xFF00 (dyn) → the pull boxes a wide element
     * for a tagged-wanting (named/dyn) callback. Var-ref callbacks and the
     * vec/map paths stay tagged. */
    uint16_t pred_enc = 0xFF00u;
    bool inline_proc = (args[1]->type == AST_COMMAND &&
                        args[1]->data.command.head_id == HEAD_PROC);
    uint32_t pred_idx = args[1]->inferred_struct_idx;
    if (inline_proc && col_type == TYPE_STREAM &&
        COMPILER_IS_SCALAR_TYPE_IDX(pred_idx)) {
    ...
    if ((argc == 2 || argc == 3) && args[1]->type == AST_LIT_STRING &&
      ...
      if (telt->type == AST_LIT_STRING) {
        ...
        if (inner_telt->type == AST_COMMAND &&
          ...
          /* Decode the LHS chain into dims[] + leaf encoding. dims[0] =
           * outer_n; dims[1..] are pulled from the chained TYPE_SHAPE_BUF
           * entries until the leaf is reached. */
          uint32_t dims[16];
          uint32_t dim_count = 1;
          dims[0] = outer_n;
          uint32_t leaf_enc = inner_shape_idx;
          while (leaf_enc < nest_reg->count &&
                 nest_reg->shapes[leaf_enc].kind == TYPE_SHAPE_BUF) {
            if (dim_count >= 16) {
              compiler__error(c, line, col,
                  "[Buf N [Buf M ...]] nesting depth exceeds 16");
              return;
            }
           ...
           * compiler__intern_buf_shape_chain for the LHS; the RHS just
           * needs to mirror it. The recursive emitter below re-validates
           * every inner [Buf N rest...] head as it descends. */
          ...
          if (argc == 3) {
            ...
            if (rhs->type != AST_COMMAND ||
              ...
              compiler__error(c, line, col,
                  "def [Buf N [Buf M ...]]: RHS must be omitted (zero-init) or "
                  "[[Buf N [Buf M ...]] ...] literal");
              return;
            }
            init_ctor = rhs;
