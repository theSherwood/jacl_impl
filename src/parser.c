...
const char* parser__parse_inline_struct_type(Parser* p, uint32_t* out_len) {
  /* Surface form: `struct [type name, type name, …]` (bracketed, type-name
   * order — same as a named struct's field list). The internal canonical
   * string stays `struct{name:type,…}` (an internal type key, never re-lexed;
   * consumed by compiler__register_inline_struct + the typer mirror). */
  parser__advance(p); /* consume 'struct' */
  ...
  while (!parser__at_end(p) && parser__peek(p)->type != TOKEN_RBRACKET) {
    ...
    /* Field TYPE first: a type word/keyword, or a nested `struct [...]`. */
    ...
    if ((tname->type == TOKEN_STRUCT ||
        ...
        p->pos + 1 < p->count &&
        p->tokens[p->pos + 1].type == TOKEN_LBRACKET) {
      uint32_t nested_len = 0;
      type_str = parser__parse_inline_struct_type(p, &nested_len);
      ...
      parser__error(p, "expected field type in inline struct field", tname);
      return NULL;
    }
    ...
    while (!parser__at_end(p) && parser__peek(p)->type != fields_close) {
      ...
      if (type_tok->type == TOKEN_WORD) {
        ...
        parser__advance(p);
                 ...
                 p->pos + 1 < p->count &&
                 p->tokens[p->pos + 1].type == TOKEN_LBRACKET) {
        /* Inline struct type: struct [type name, …] */
