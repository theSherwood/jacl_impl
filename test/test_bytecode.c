...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_STRUCT_STORE_INLINE, 131);
  ASSERT_INT_EQ(OP_STRUCT_GET_UPVALUE, 132);
  ASSERT_INT_EQ(OP_STRUCT_SET_UPVALUE, 133);
  ASSERT_INT_EQ(OP_LOAD_INLINE_LOCAL,   134);
  ASSERT_INT_EQ(OP_LOAD_INLINE_UPVALUE, 135);
  ASSERT_INT_EQ(OP_PRINT_STRUCT,       136);
  ASSERT_INT_EQ(OP_STRUCT_EQ_TOS,      137);
  ASSERT_INT_EQ(OP_STRUCT_GET_INLINE_TOS, 138);
  ASSERT_INT_EQ(OP_STRUCT_EXPAND,      139);
  ASSERT_INT_EQ(OP_STRUCT_EQ_INLINE,   140);
  ASSERT_INT_EQ(OP_STRUCT_HASH_INLINE, 141);
  ASSERT_INT_EQ(OP_HASH,               142);
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        143);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  144);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 145);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 146);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 147);
  ASSERT_INT_EQ(OP_SPREAD,           148);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      149);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      150);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 151);
  ASSERT_INT_EQ(OP_YIELD,            152);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      153);
  ASSERT_INT_EQ(OP_COLLECT,          154);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 155);
  ASSERT_INT_EQ(OP_COUNT,            156);
  ASSERT_INT_EQ(OP_TAKE,             157);
  ASSERT_INT_EQ(OP_FIRST,            158);
  ASSERT_INT_EQ(OP_LINES,            159);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  160);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  161);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 162);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 163);
  ASSERT_INT_EQ(OP_YIELD_SM,        164);
  ASSERT_INT_EQ(OP_AWAIT_SM,        165);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   166);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 167);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 168);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 169);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 170);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   171);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       172);
  ASSERT_INT_EQ(OP_INTERPRET,       173);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 174);
  ASSERT_INT_EQ(OP_EXEC,             175);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        176);
  ASSERT_INT_EQ(OP_SIGNAL,           177);
  ASSERT_INT_EQ(OP_HALT,             178);
  TEST_PASS();
}
