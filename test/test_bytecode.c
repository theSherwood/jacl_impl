...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        126);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  127);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 128);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 129);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 130);
  ASSERT_INT_EQ(OP_SPREAD,           131);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      132);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      133);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 134);
  ASSERT_INT_EQ(OP_YIELD,            135);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      136);
  ASSERT_INT_EQ(OP_COLLECT,          137);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 138);
  ASSERT_INT_EQ(OP_COUNT,            139);
  ASSERT_INT_EQ(OP_TAKE,             140);
  ASSERT_INT_EQ(OP_FIRST,            141);
  ASSERT_INT_EQ(OP_LINES,            142);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  143);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  144);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 145);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 146);
  ASSERT_INT_EQ(OP_YIELD_SM,        147);
  ASSERT_INT_EQ(OP_AWAIT_SM,        148);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   149);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 150);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 151);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 152);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 153);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   154);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       155);
  ASSERT_INT_EQ(OP_INTERPRET,       156);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 157);
  ASSERT_INT_EQ(OP_EXEC,             158);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        159);
  ASSERT_INT_EQ(OP_SIGNAL,           160);
  ASSERT_INT_EQ(OP_HALT,             161);
  TEST_PASS();
}
