...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   152);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       153);
  ASSERT_INT_EQ(OP_INTERPRET,       154);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 155);
  ASSERT_INT_EQ(OP_EXEC,             156);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        157);
  ASSERT_INT_EQ(OP_SIGNAL,           158);
  ASSERT_INT_EQ(OP_HALT,             159);
  TEST_PASS();
}
