...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_STRUCT_NEW_INLINE, 130);
  ASSERT_INT_EQ(OP_STRUCT_GET_INLINE, 131);
  ASSERT_INT_EQ(OP_STRUCT_SET_INLINE, 132);
  ASSERT_INT_EQ(OP_STRUCT_STORE_INLINE, 133);
  ASSERT_INT_EQ(OP_STRUCT_GET_UPVALUE, 134);
  ASSERT_INT_EQ(OP_STRUCT_SET_UPVALUE, 135);
  ASSERT_INT_EQ(OP_LOAD_INLINE_LOCAL,   136);
  ASSERT_INT_EQ(OP_LOAD_INLINE_UPVALUE, 137);
  ASSERT_INT_EQ(OP_PRINT_STRUCT,       138);
  ASSERT_INT_EQ(OP_STRUCT_EQ_TOS,      139);
  ASSERT_INT_EQ(OP_STRUCT_GET_INLINE_TOS, 140);
  ASSERT_INT_EQ(OP_STRUCT_EXPAND,      141);
  ASSERT_INT_EQ(OP_STRUCT_EQ_INLINE,   142);
  ASSERT_INT_EQ(OP_STRUCT_HASH_INLINE, 143);
  ASSERT_INT_EQ(OP_HASH,               144);
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        145);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  146);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 147);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 148);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 149);
  ASSERT_INT_EQ(OP_SPREAD,           150);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      151);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      152);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 153);
  ASSERT_INT_EQ(OP_YIELD,            154);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      155);
  ASSERT_INT_EQ(OP_COLLECT,          156);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 157);
  ASSERT_INT_EQ(OP_COUNT,            158);
  ASSERT_INT_EQ(OP_TAKE,             159);
  ASSERT_INT_EQ(OP_FIRST,            160);
  ASSERT_INT_EQ(OP_LINES,            161);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  162);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  163);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 164);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 165);
  ASSERT_INT_EQ(OP_YIELD_SM,        166);
  ASSERT_INT_EQ(OP_AWAIT_SM,        167);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   168);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 169);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 170);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 171);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 172);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   173);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       174);
  ASSERT_INT_EQ(OP_INTERPRET,       175);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 176);
  ASSERT_INT_EQ(OP_EXEC,             177);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        178);
  ASSERT_INT_EQ(OP_SIGNAL,           179);
  ASSERT_INT_EQ(OP_HALT,             180);
  TEST_PASS();
}
