...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_VEC_SPREAD,     43);
  /* M8 map opcodes */
  ASSERT_INT_EQ(OP_MAP,            44);
  ASSERT_INT_EQ(OP_MAP_GET,        45);
  ASSERT_INT_EQ(OP_MAP_HAS,        46);
  ASSERT_INT_EQ(OP_MAP_LEN,        47);
  ASSERT_INT_EQ(OP_MAP_SET,        48);
  ASSERT_INT_EQ(OP_MAP_REMOVE,     49);
  ASSERT_INT_EQ(OP_MAP_KEYS,       50);
  ASSERT_INT_EQ(OP_MAP_VALS,       51);
  ASSERT_INT_EQ(OP_EACH,           52);
  ASSERT_INT_EQ(OP_TRANSFORM,      53);
  ASSERT_INT_EQ(OP_FILTER,         54);
  /* M9 error handling opcodes */
  ASSERT_INT_EQ(OP_ERROR,          55);
  ASSERT_INT_EQ(OP_IS_ERROR,       56);
  ASSERT_INT_EQ(OP_ERROR_VAL,      57);
  ASSERT_INT_EQ(OP_CHECK_ERROR,    58);
  ASSERT_INT_EQ(OP_JUMP_IF_ERROR,  59);
  ASSERT_INT_EQ(OP_STACK_TRACE,    60);
  /* M10 mutable state opcodes */
  ASSERT_INT_EQ(OP_MAKE_CELL,       61);
  ASSERT_INT_EQ(OP_GET_CELL_LOCAL,   62);
  ASSERT_INT_EQ(OP_SET_CELL_LOCAL,   63);
  ASSERT_INT_EQ(OP_GET_CELL_UPVALUE, 64);
  ASSERT_INT_EQ(OP_SET_CELL_UPVALUE, 65);
  ASSERT_INT_EQ(OP_SET_GLOBAL,      66);
  ASSERT_INT_EQ(OP_BOX,             67);
  ASSERT_INT_EQ(OP_BOX_STRUCT,      68);
  ASSERT_INT_EQ(OP_ATOM,            69);
  ASSERT_INT_EQ(OP_DEREF,           70);
  ASSERT_INT_EQ(OP_RESET,           71);
  ASSERT_INT_EQ(OP_SWAP,            72);
  ASSERT_INT_EQ(OP_IS_BOX,          73);
  ASSERT_INT_EQ(OP_IS_BOX_TYPED,    74);
  ASSERT_INT_EQ(OP_IS_ATOM,         75);
  ASSERT_INT_EQ(OP_IS_FUTURE,       76);
  /* M13 CPS and concurrency opcodes */
  ASSERT_INT_EQ(OP_AWAIT,           77);
  ASSERT_INT_EQ(OP_SPAWN,           78);
  ASSERT_INT_EQ(OP_RESOLVE_FUTURE,  79);
  ASSERT_INT_EQ(OP_PARALLEL,        80);
  ASSERT_INT_EQ(OP_RACE,            81);
  ASSERT_INT_EQ(OP_COMPLETE_PARALLEL, 82);
  ASSERT_INT_EQ(OP_COMPLETE_RACE,    83);
  /* M11 typed opcodes */
  ASSERT_INT_EQ(OP_ADD_I64,         84);
  ASSERT_INT_EQ(OP_SUB_I64,         85);
  ASSERT_INT_EQ(OP_MUL_I64,         86);
  ASSERT_INT_EQ(OP_DIV_I64,         87);
  ASSERT_INT_EQ(OP_MOD_I64,         88);
  ASSERT_INT_EQ(OP_NEG_I64,         89);
  ASSERT_INT_EQ(OP_LT_I64,          90);
  ASSERT_INT_EQ(OP_GT_I64,          91);
  ASSERT_INT_EQ(OP_LE_I64,          92);
  ASSERT_INT_EQ(OP_GE_I64,          93);
  ASSERT_INT_EQ(OP_EQ_I64,          94);
  ASSERT_INT_EQ(OP_DIV_U64,         95);
  ASSERT_INT_EQ(OP_MOD_U64,         96);
  ASSERT_INT_EQ(OP_LT_U64,          97);
  ASSERT_INT_EQ(OP_GT_U64,          98);
  ASSERT_INT_EQ(OP_LE_U64,          99);
  ASSERT_INT_EQ(OP_GE_U64,        100);
  ASSERT_INT_EQ(OP_ADD_F64,        101);
  ASSERT_INT_EQ(OP_SUB_F64,        102);
  ASSERT_INT_EQ(OP_MUL_F64,        103);
  ASSERT_INT_EQ(OP_DIV_F64,        104);
  ASSERT_INT_EQ(OP_MOD_F64,        105);
  ASSERT_INT_EQ(OP_NEG_F64,        106);
  ASSERT_INT_EQ(OP_LT_F64,         107);
  ASSERT_INT_EQ(OP_GT_F64,         108);
  ASSERT_INT_EQ(OP_LE_F64,         109);
  ASSERT_INT_EQ(OP_GE_F64,         110);
  ASSERT_INT_EQ(OP_EQ_F64,         111);
  ASSERT_INT_EQ(OP_TO_I32,         112);
  ASSERT_INT_EQ(OP_TO_I64,         113);
  ASSERT_INT_EQ(OP_TO_U32,         114);
  ASSERT_INT_EQ(OP_TO_U64,         115);
  ASSERT_INT_EQ(OP_TO_F32,         116);
  ASSERT_INT_EQ(OP_TO_F64,         117);
  ASSERT_INT_EQ(OP_TO_DYN,         118);
  ASSERT_INT_EQ(OP_CONST_I64,      119);
  ASSERT_INT_EQ(OP_CONST_U64,      120);
  ASSERT_INT_EQ(OP_CONST_F64,      121);
  ASSERT_INT_EQ(OP_STRUCT_NEW,     122);
  ASSERT_INT_EQ(OP_STRUCT_GET,     123);
  ASSERT_INT_EQ(OP_STRUCT_SET,     124);
  ASSERT_INT_EQ(OP_STRUCT_GET_DYN, 125);
  ASSERT_INT_EQ(OP_STRUCT_SET_DYN, 126);
  ASSERT_INT_EQ(OP_STRUCT_NEW_INLINE, 127);
  ASSERT_INT_EQ(OP_STRUCT_GET_INLINE, 128);
  ASSERT_INT_EQ(OP_STRUCT_SET_INLINE, 129);
  ASSERT_INT_EQ(OP_STRUCT_MATERIALIZE, 130);
  ASSERT_INT_EQ(OP_STRUCT_COPY,       131);
  ASSERT_INT_EQ(OP_STRUCT_STORE_INLINE, 132);
  ASSERT_INT_EQ(OP_STRUCT_GET_UPVALUE, 133);
  ASSERT_INT_EQ(OP_STRUCT_SET_UPVALUE, 134);
  ASSERT_INT_EQ(OP_STRUCT_MATERIALIZE_UPVALUE, 135);
  ASSERT_INT_EQ(OP_STRUCT_EXPAND,      136);
  ASSERT_INT_EQ(OP_STRUCT_EQ_INLINE,   137);
  ASSERT_INT_EQ(OP_STRUCT_HASH_INLINE, 138);
  ASSERT_INT_EQ(OP_HASH,               139);
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        140);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  141);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 142);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 143);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 144);
  ASSERT_INT_EQ(OP_SPREAD,           145);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      146);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      147);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 148);
  ASSERT_INT_EQ(OP_YIELD,            149);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      150);
  ASSERT_INT_EQ(OP_COLLECT,          151);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 152);
  ASSERT_INT_EQ(OP_COUNT,            153);
  ASSERT_INT_EQ(OP_TAKE,             154);
  ASSERT_INT_EQ(OP_FIRST,            155);
  ASSERT_INT_EQ(OP_LINES,            156);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  157);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  158);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 159);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 160);
  ASSERT_INT_EQ(OP_YIELD_SM,        161);
  ASSERT_INT_EQ(OP_AWAIT_SM,        162);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   163);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 164);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 165);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 166);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 167);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   168);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       169);
  ASSERT_INT_EQ(OP_INTERPRET,       170);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 171);
  ASSERT_INT_EQ(OP_EXEC,             172);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        173);
  ASSERT_INT_EQ(OP_SIGNAL,           174);
  ASSERT_INT_EQ(OP_HALT,             175);
  TEST_PASS();
}
