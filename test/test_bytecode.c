...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 127);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 128);
  ...
  ASSERT_INT_EQ(OP_YIELD,            133);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      134);
  ASSERT_INT_EQ(OP_COLLECT,          135);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 136);
  ASSERT_INT_EQ(OP_COUNT,            137);
  ASSERT_INT_EQ(OP_TAKE,             138);
  ASSERT_INT_EQ(OP_FIRST,            139);
  ASSERT_INT_EQ(OP_LINES,            140);
  ASSERT_INT_EQ(OP_HALT,             141);
  TEST_PASS();
}
