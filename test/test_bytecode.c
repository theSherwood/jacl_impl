...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_RETURN_WIDE,    28);
  ASSERT_INT_EQ(OP_CLOSURE,        29);
  ASSERT_INT_EQ(OP_POP_N,          30);
  /* M5 opcodes */
  ASSERT_INT_EQ(OP_CONCAT,         31);
  ASSERT_INT_EQ(OP_STR_LEN,        32);
  ASSERT_INT_EQ(OP_STR_BYTE_LEN,   33);
  ASSERT_INT_EQ(OP_STR_INDEX,      34);
  ASSERT_INT_EQ(OP_STR_SLICE,      35);
  ASSERT_INT_EQ(OP_TO_STRING,      36);
  /* M8 opcodes */
  ASSERT_INT_EQ(OP_VEC,            37);
  ASSERT_INT_EQ(OP_VEC_GET,        38);
  ASSERT_INT_EQ(OP_VEC_LEN,        39);
  ASSERT_INT_EQ(OP_VEC_PUSH,       40);
  ASSERT_INT_EQ(OP_VEC_SET,        41);
  ASSERT_INT_EQ(OP_VEC_CONCAT,     42);
  ASSERT_INT_EQ(OP_VEC_SLICE,      43);
  ASSERT_INT_EQ(OP_VEC_SPREAD,     44);
  /* M8 map opcodes */
  ASSERT_INT_EQ(OP_MAP,            45);
  ASSERT_INT_EQ(OP_MAP_GET,        46);
  ASSERT_INT_EQ(OP_MAP_HAS,        47);
  ASSERT_INT_EQ(OP_MAP_LEN,        48);
  ASSERT_INT_EQ(OP_MAP_SET,        49);
  ASSERT_INT_EQ(OP_MAP_REMOVE,     50);
  ASSERT_INT_EQ(OP_MAP_KEYS,       51);
  ASSERT_INT_EQ(OP_MAP_VALS,       52);
  ASSERT_INT_EQ(OP_EACH,           53);
  ASSERT_INT_EQ(OP_TRANSFORM,      54);
  ASSERT_INT_EQ(OP_FILTER,         55);
  /* M9 error handling opcodes */
  ASSERT_INT_EQ(OP_ERROR,          56);
  ASSERT_INT_EQ(OP_IS_ERROR,       57);
  ASSERT_INT_EQ(OP_ERROR_VAL,      58);
  ASSERT_INT_EQ(OP_CHECK_ERROR,    59);
  ASSERT_INT_EQ(OP_JUMP_IF_ERROR,  60);
  ASSERT_INT_EQ(OP_STACK_TRACE,    61);
  /* M10 mutable state opcodes */
  ASSERT_INT_EQ(OP_MAKE_CELL,       62);
  ASSERT_INT_EQ(OP_GET_CELL_LOCAL,   63);
  ASSERT_INT_EQ(OP_SET_CELL_LOCAL,   64);
  ASSERT_INT_EQ(OP_GET_CELL_UPVALUE, 65);
  ASSERT_INT_EQ(OP_SET_CELL_UPVALUE, 66);
  ASSERT_INT_EQ(OP_SET_GLOBAL,      67);
  ASSERT_INT_EQ(OP_BOX,             68);
  ASSERT_INT_EQ(OP_BOX_STRUCT,      69);
  ASSERT_INT_EQ(OP_ATOM,            70);
  ASSERT_INT_EQ(OP_DEREF,           71);
  ASSERT_INT_EQ(OP_RESET,           72);
  ASSERT_INT_EQ(OP_SWAP,            73);
  ASSERT_INT_EQ(OP_IS_BOX,          74);
  ASSERT_INT_EQ(OP_IS_BOX_TYPED,    75);
  ASSERT_INT_EQ(OP_IS_ATOM,         76);
  ASSERT_INT_EQ(OP_IS_FUTURE,       77);
  /* M13 CPS and concurrency opcodes */
  ASSERT_INT_EQ(OP_AWAIT,           78);
  ASSERT_INT_EQ(OP_SPAWN,           79);
  ASSERT_INT_EQ(OP_RESOLVE_FUTURE,  80);
  ASSERT_INT_EQ(OP_PARALLEL,        81);
  ASSERT_INT_EQ(OP_RACE,            82);
  ASSERT_INT_EQ(OP_COMPLETE_PARALLEL, 83);
  ASSERT_INT_EQ(OP_COMPLETE_RACE,    84);
  /* M11 typed opcodes */
  ASSERT_INT_EQ(OP_ADD_I64,         85);
  ASSERT_INT_EQ(OP_SUB_I64,         86);
  ASSERT_INT_EQ(OP_MUL_I64,         87);
  ASSERT_INT_EQ(OP_DIV_I64,         88);
  ASSERT_INT_EQ(OP_MOD_I64,         89);
  ASSERT_INT_EQ(OP_NEG_I64,         90);
  ASSERT_INT_EQ(OP_LT_I64,          91);
  ASSERT_INT_EQ(OP_GT_I64,          92);
  ASSERT_INT_EQ(OP_LE_I64,          93);
  ASSERT_INT_EQ(OP_GE_I64,          94);
  ASSERT_INT_EQ(OP_EQ_I64,          95);
  ASSERT_INT_EQ(OP_DIV_U64,         96);
  ASSERT_INT_EQ(OP_MOD_U64,         97);
  ASSERT_INT_EQ(OP_LT_U64,          98);
  ASSERT_INT_EQ(OP_GT_U64,          99);
  ASSERT_INT_EQ(OP_LE_U64,         100);
  ASSERT_INT_EQ(OP_GE_U64,        101);
  ASSERT_INT_EQ(OP_ADD_F64,        102);
  ASSERT_INT_EQ(OP_SUB_F64,        103);
  ASSERT_INT_EQ(OP_MUL_F64,        104);
  ASSERT_INT_EQ(OP_DIV_F64,        105);
  ASSERT_INT_EQ(OP_MOD_F64,        106);
  ASSERT_INT_EQ(OP_NEG_F64,        107);
  ASSERT_INT_EQ(OP_LT_F64,         108);
  ASSERT_INT_EQ(OP_GT_F64,         109);
  ASSERT_INT_EQ(OP_LE_F64,         110);
  ASSERT_INT_EQ(OP_GE_F64,         111);
  ASSERT_INT_EQ(OP_EQ_F64,         112);
  ASSERT_INT_EQ(OP_TO_I32,         113);
  ASSERT_INT_EQ(OP_TO_I64,         114);
  ASSERT_INT_EQ(OP_TO_U32,         115);
  ASSERT_INT_EQ(OP_TO_U64,         116);
  ASSERT_INT_EQ(OP_TO_F32,         117);
  ASSERT_INT_EQ(OP_TO_F64,         118);
  ASSERT_INT_EQ(OP_TO_DYN,         119);
  ASSERT_INT_EQ(OP_CONST_I64,      120);
  ASSERT_INT_EQ(OP_CONST_U64,      121);
  ASSERT_INT_EQ(OP_CONST_F64,      122);
  ASSERT_INT_EQ(OP_HEAP_RECORD_NEW,     123);
  ASSERT_INT_EQ(OP_HEAP_RECORD_GET,     124);
  ASSERT_INT_EQ(OP_HEAP_RECORD_SET,     125);
  ASSERT_INT_EQ(OP_HEAP_RECORD_GET_DYN, 126);
  ASSERT_INT_EQ(OP_HEAP_RECORD_SET_DYN, 127);
  ASSERT_INT_EQ(OP_HEAP_RECORD_GET_INLINE, 128);
  ASSERT_INT_EQ(OP_HEAP_RECORD_SET_INLINE, 129);
  ASSERT_INT_EQ(OP_RESET_INLINE,           130);
  ASSERT_INT_EQ(OP_STRUCT_NEW_INLINE, 131);
  ASSERT_INT_EQ(OP_STRUCT_GET_INLINE, 132);
  ASSERT_INT_EQ(OP_STRUCT_SET_INLINE, 133);
  ASSERT_INT_EQ(OP_STRUCT_STORE_INLINE, 134);
  ASSERT_INT_EQ(OP_STRUCT_GET_UPVALUE, 135);
  ASSERT_INT_EQ(OP_STRUCT_SET_UPVALUE, 136);
  ASSERT_INT_EQ(OP_LOAD_INLINE_LOCAL,   137);
  ASSERT_INT_EQ(OP_LOAD_INLINE_UPVALUE, 138);
  ASSERT_INT_EQ(OP_PRINT_STRUCT,       139);
  ASSERT_INT_EQ(OP_STRUCT_EQ_TOS,      140);
  ASSERT_INT_EQ(OP_STRUCT_GET_INLINE_TOS, 141);
  ASSERT_INT_EQ(OP_STRUCT_EXPAND,      142);
  ASSERT_INT_EQ(OP_STRUCT_EQ_INLINE,   143);
  ASSERT_INT_EQ(OP_STRUCT_HASH_INLINE, 144);
  ASSERT_INT_EQ(OP_HASH,               145);
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        146);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  147);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 148);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 149);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 150);
  ASSERT_INT_EQ(OP_SPREAD,           151);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      152);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      153);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 154);
  ASSERT_INT_EQ(OP_YIELD,            155);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      156);
  ASSERT_INT_EQ(OP_COLLECT,          157);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 158);
  ASSERT_INT_EQ(OP_COUNT,            159);
  ASSERT_INT_EQ(OP_TAKE,             160);
  ASSERT_INT_EQ(OP_FIRST,            161);
  ASSERT_INT_EQ(OP_LINES,            162);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  163);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  164);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 165);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 166);
  ASSERT_INT_EQ(OP_YIELD_SM,        167);
  ASSERT_INT_EQ(OP_AWAIT_SM,        168);
  ASSERT_INT_EQ(OP_SLEEP_SM,        169);
  ASSERT_INT_EQ(OP_SLEEP_BLOCK,     170);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,    171);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 172);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 173);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 174);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 175);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   176);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       177);
  ASSERT_INT_EQ(OP_INTERPRET,       178);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 179);
  ASSERT_INT_EQ(OP_EXEC,             180);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        181);
  ASSERT_INT_EQ(OP_SIGNAL,           182);
  ASSERT_INT_EQ(OP_HALT,             183);
  TEST_PASS();
}
