...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_STRUCT_EQ_INLINE,   136);
  ASSERT_INT_EQ(OP_STRUCT_HASH_INLINE, 137);
  ASSERT_INT_EQ(OP_HASH,               138);
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        139);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  140);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 141);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 142);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 143);
  ASSERT_INT_EQ(OP_SPREAD,           144);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      145);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      146);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 147);
  ASSERT_INT_EQ(OP_YIELD,            148);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      149);
  ASSERT_INT_EQ(OP_COLLECT,          150);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 151);
  ASSERT_INT_EQ(OP_COUNT,            152);
  ASSERT_INT_EQ(OP_TAKE,             153);
  ASSERT_INT_EQ(OP_FIRST,            154);
  ASSERT_INT_EQ(OP_LINES,            155);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  156);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  157);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 158);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 159);
  ASSERT_INT_EQ(OP_YIELD_SM,        160);
  ASSERT_INT_EQ(OP_AWAIT_SM,        161);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   162);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 163);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 164);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 165);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 166);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   167);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       168);
  ASSERT_INT_EQ(OP_INTERPRET,       169);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 170);
  ASSERT_INT_EQ(OP_EXEC,             171);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        172);
  ASSERT_INT_EQ(OP_SIGNAL,           173);
  ASSERT_INT_EQ(OP_HALT,             174);
  TEST_PASS();
}
