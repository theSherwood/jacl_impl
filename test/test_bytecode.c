...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        131);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  132);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 133);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 134);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 135);
  ASSERT_INT_EQ(OP_SPREAD,           136);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      137);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      138);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 139);
  ASSERT_INT_EQ(OP_YIELD,            140);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      141);
  ASSERT_INT_EQ(OP_COLLECT,          142);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 143);
  ASSERT_INT_EQ(OP_COUNT,            144);
  ASSERT_INT_EQ(OP_TAKE,             145);
  ASSERT_INT_EQ(OP_FIRST,            146);
  ASSERT_INT_EQ(OP_LINES,            147);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  148);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  149);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 150);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 151);
  ASSERT_INT_EQ(OP_YIELD_SM,        152);
  ASSERT_INT_EQ(OP_AWAIT_SM,        153);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   154);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 155);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 156);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 157);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 158);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   159);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       160);
  ASSERT_INT_EQ(OP_INTERPRET,       161);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 162);
  ASSERT_INT_EQ(OP_EXEC,             163);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        164);
  ASSERT_INT_EQ(OP_SIGNAL,           165);
  ASSERT_INT_EQ(OP_HALT,             166);
  TEST_PASS();
}
