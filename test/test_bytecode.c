...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_STRUCT_STORE_INLINE, 132);
  ASSERT_INT_EQ(OP_STRUCT_GET_UPVALUE, 133);
  ASSERT_INT_EQ(OP_STRUCT_SET_UPVALUE, 134);
  ASSERT_INT_EQ(OP_LOAD_INLINE_LOCAL,   135);
  ASSERT_INT_EQ(OP_LOAD_INLINE_UPVALUE, 136);
  ASSERT_INT_EQ(OP_PRINT_STRUCT,       137);
  ASSERT_INT_EQ(OP_STRUCT_EQ_TOS,      138);
  ASSERT_INT_EQ(OP_STRUCT_GET_INLINE_TOS, 139);
  ASSERT_INT_EQ(OP_STRUCT_EXPAND,      140);
  ASSERT_INT_EQ(OP_STRUCT_EQ_INLINE,   141);
  ASSERT_INT_EQ(OP_STRUCT_HASH_INLINE, 142);
  ASSERT_INT_EQ(OP_HASH,               143);
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        144);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  145);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 146);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 147);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 148);
  ASSERT_INT_EQ(OP_SPREAD,           149);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      150);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      151);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 152);
  ASSERT_INT_EQ(OP_YIELD,            153);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      154);
  ASSERT_INT_EQ(OP_COLLECT,          155);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 156);
  ASSERT_INT_EQ(OP_COUNT,            157);
  ASSERT_INT_EQ(OP_TAKE,             158);
  ASSERT_INT_EQ(OP_FIRST,            159);
  ASSERT_INT_EQ(OP_LINES,            160);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  161);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  162);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 163);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 164);
  ASSERT_INT_EQ(OP_YIELD_SM,        165);
  ASSERT_INT_EQ(OP_AWAIT_SM,        166);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   167);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 168);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 169);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 170);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 171);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   172);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       173);
  ASSERT_INT_EQ(OP_INTERPRET,       174);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 175);
  ASSERT_INT_EQ(OP_EXEC,             176);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        177);
  ASSERT_INT_EQ(OP_SIGNAL,           178);
  ASSERT_INT_EQ(OP_HALT,             179);
  TEST_PASS();
}
