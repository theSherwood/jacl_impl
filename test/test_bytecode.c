...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_STR_INDEX,      32);
  ASSERT_INT_EQ(OP_STR_SLICE,      33);
  ASSERT_INT_EQ(OP_TO_STRING,      34);
  /* M8 opcodes */
  ASSERT_INT_EQ(OP_VEC,            35);
  ASSERT_INT_EQ(OP_VEC_GET,        36);
  ASSERT_INT_EQ(OP_VEC_LEN,        37);
  ASSERT_INT_EQ(OP_VEC_PUSH,       38);
  ASSERT_INT_EQ(OP_VEC_SET,        39);
  ASSERT_INT_EQ(OP_VEC_CONCAT,     40);
  ASSERT_INT_EQ(OP_VEC_SLICE,      41);
  /* M8 map opcodes */
  ASSERT_INT_EQ(OP_MAP,            42);
  ASSERT_INT_EQ(OP_MAP_GET,        43);
  ASSERT_INT_EQ(OP_MAP_HAS,        44);
  ASSERT_INT_EQ(OP_MAP_LEN,        45);
  ASSERT_INT_EQ(OP_MAP_SET,        46);
  ASSERT_INT_EQ(OP_MAP_REMOVE,     47);
  ASSERT_INT_EQ(OP_MAP_KEYS,       48);
  ASSERT_INT_EQ(OP_MAP_VALS,       49);
  ASSERT_INT_EQ(OP_EACH,           50);
  ASSERT_INT_EQ(OP_TRANSFORM,      51);
  ASSERT_INT_EQ(OP_FILTER,         52);
  /* M9 error handling opcodes */
  ASSERT_INT_EQ(OP_ERROR,          53);
  ASSERT_INT_EQ(OP_IS_ERROR,       54);
  ASSERT_INT_EQ(OP_ERROR_VAL,      55);
  ASSERT_INT_EQ(OP_CHECK_ERROR,    56);
  ASSERT_INT_EQ(OP_JUMP_IF_ERROR,  57);
  ASSERT_INT_EQ(OP_STACK_TRACE,    58);
  /* M10 mutable state opcodes */
  ASSERT_INT_EQ(OP_MAKE_CELL,       59);
  ASSERT_INT_EQ(OP_GET_CELL_LOCAL,   60);
  ASSERT_INT_EQ(OP_SET_CELL_LOCAL,   61);
  ASSERT_INT_EQ(OP_GET_CELL_UPVALUE, 62);
  ASSERT_INT_EQ(OP_SET_CELL_UPVALUE, 63);
  ASSERT_INT_EQ(OP_SET_GLOBAL,      64);
  ASSERT_INT_EQ(OP_BOX,             65);
  ASSERT_INT_EQ(OP_ATOM,            66);
  ASSERT_INT_EQ(OP_DEREF,           67);
  ASSERT_INT_EQ(OP_RESET,           68);
  ASSERT_INT_EQ(OP_SWAP,            69);
  ASSERT_INT_EQ(OP_IS_BOX,          70);
  ASSERT_INT_EQ(OP_IS_ATOM,         71);
  ASSERT_INT_EQ(OP_IS_FUTURE,       72);
  /* M13 CPS and concurrency opcodes */
  ASSERT_INT_EQ(OP_AWAIT,           73);
  ASSERT_INT_EQ(OP_SPAWN,           74);
  ASSERT_INT_EQ(OP_RESOLVE_FUTURE,  75);
  ASSERT_INT_EQ(OP_PARALLEL,        76);
  ASSERT_INT_EQ(OP_RACE,            77);
  ASSERT_INT_EQ(OP_COMPLETE_PARALLEL, 78);
  ASSERT_INT_EQ(OP_COMPLETE_RACE,    79);
  /* M11 typed opcodes */
  ASSERT_INT_EQ(OP_ADD_I64,         80);
  ASSERT_INT_EQ(OP_SUB_I64,         81);
  ASSERT_INT_EQ(OP_MUL_I64,         82);
  ASSERT_INT_EQ(OP_DIV_I64,         83);
  ASSERT_INT_EQ(OP_MOD_I64,         84);
  ASSERT_INT_EQ(OP_NEG_I64,         85);
  ASSERT_INT_EQ(OP_LT_I64,          86);
  ASSERT_INT_EQ(OP_GT_I64,          87);
  ASSERT_INT_EQ(OP_LE_I64,          88);
  ASSERT_INT_EQ(OP_GE_I64,          89);
  ASSERT_INT_EQ(OP_EQ_I64,          90);
  ASSERT_INT_EQ(OP_DIV_U64,         91);
  ASSERT_INT_EQ(OP_MOD_U64,         92);
  ASSERT_INT_EQ(OP_LT_U64,          93);
  ASSERT_INT_EQ(OP_GT_U64,          94);
  ASSERT_INT_EQ(OP_LE_U64,          95);
  ASSERT_INT_EQ(OP_GE_U64,          96);
  ASSERT_INT_EQ(OP_ADD_F64,         97);
  ASSERT_INT_EQ(OP_SUB_F64,         98);
  ASSERT_INT_EQ(OP_MUL_F64,         99);
  ASSERT_INT_EQ(OP_DIV_F64,        100);
  ASSERT_INT_EQ(OP_MOD_F64,        101);
  ASSERT_INT_EQ(OP_NEG_F64,        102);
  ASSERT_INT_EQ(OP_LT_F64,         103);
  ASSERT_INT_EQ(OP_GT_F64,         104);
  ASSERT_INT_EQ(OP_LE_F64,         105);
  ASSERT_INT_EQ(OP_GE_F64,         106);
  ASSERT_INT_EQ(OP_EQ_F64,         107);
  ASSERT_INT_EQ(OP_TO_I32,         108);
  ASSERT_INT_EQ(OP_TO_I64,         109);
  ASSERT_INT_EQ(OP_TO_U32,         110);
  ASSERT_INT_EQ(OP_TO_U64,         111);
  ASSERT_INT_EQ(OP_TO_F32,         112);
  ASSERT_INT_EQ(OP_TO_F64,         113);
  ASSERT_INT_EQ(OP_TO_DYN,         114);
  ASSERT_INT_EQ(OP_CONST_I64,      115);
  ASSERT_INT_EQ(OP_CONST_U64,      116);
  ASSERT_INT_EQ(OP_CONST_F64,      117);
  ASSERT_INT_EQ(OP_STRUCT_NEW,     118);
  ASSERT_INT_EQ(OP_STRUCT_GET,     119);
  ASSERT_INT_EQ(OP_HALT,           120);
  TEST_PASS();
}
