...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_STRUCT_EXPAND,      137);
  ASSERT_INT_EQ(OP_STRUCT_EQ_INLINE,   138);
  ASSERT_INT_EQ(OP_STRUCT_HASH_INLINE, 139);
  ASSERT_INT_EQ(OP_HASH,               140);
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        141);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  142);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 143);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 144);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 145);
  ASSERT_INT_EQ(OP_SPREAD,           146);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      147);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      148);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 149);
  ASSERT_INT_EQ(OP_YIELD,            150);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      151);
  ASSERT_INT_EQ(OP_COLLECT,          152);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 153);
  ASSERT_INT_EQ(OP_COUNT,            154);
  ASSERT_INT_EQ(OP_TAKE,             155);
  ASSERT_INT_EQ(OP_FIRST,            156);
  ASSERT_INT_EQ(OP_LINES,            157);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  158);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  159);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 160);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 161);
  ASSERT_INT_EQ(OP_YIELD_SM,        162);
  ASSERT_INT_EQ(OP_AWAIT_SM,        163);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   164);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 165);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 166);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 167);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 168);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   169);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       170);
  ASSERT_INT_EQ(OP_INTERPRET,       171);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 172);
  ASSERT_INT_EQ(OP_EXEC,             173);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        174);
  ASSERT_INT_EQ(OP_SIGNAL,           175);
  ASSERT_INT_EQ(OP_HALT,             176);
  TEST_PASS();
}
