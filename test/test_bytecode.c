...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        129);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  130);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 131);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 132);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 133);
  ASSERT_INT_EQ(OP_SPREAD,           134);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      135);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      136);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 137);
  ASSERT_INT_EQ(OP_YIELD,            138);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      139);
  ASSERT_INT_EQ(OP_COLLECT,          140);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 141);
  ASSERT_INT_EQ(OP_COUNT,            142);
  ASSERT_INT_EQ(OP_TAKE,             143);
  ASSERT_INT_EQ(OP_FIRST,            144);
  ASSERT_INT_EQ(OP_LINES,            145);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  146);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  147);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 148);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 149);
  ASSERT_INT_EQ(OP_YIELD_SM,        150);
  ASSERT_INT_EQ(OP_AWAIT_SM,        151);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   152);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 153);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 154);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 155);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 156);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   157);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       158);
  ASSERT_INT_EQ(OP_INTERPRET,       159);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 160);
  ASSERT_INT_EQ(OP_EXEC,             161);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        162);
  ASSERT_INT_EQ(OP_SIGNAL,           163);
  ASSERT_INT_EQ(OP_HALT,             164);
  TEST_PASS();
}
