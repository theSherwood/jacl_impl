...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        125);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  126);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 127);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 128);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 129);
  ASSERT_INT_EQ(OP_SPREAD,           130);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      131);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      132);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 133);
  ASSERT_INT_EQ(OP_YIELD,            134);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      135);
  ASSERT_INT_EQ(OP_COLLECT,          136);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 137);
  ASSERT_INT_EQ(OP_COUNT,            138);
  ASSERT_INT_EQ(OP_TAKE,             139);
  ASSERT_INT_EQ(OP_FIRST,            140);
  ASSERT_INT_EQ(OP_LINES,            141);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  142);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  143);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 144);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 145);
  ASSERT_INT_EQ(OP_YIELD_SM,        146);
  ASSERT_INT_EQ(OP_AWAIT_SM,        147);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   148);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 149);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 150);
  ...
  ASSERT_INT_EQ(OP_EXEC,             155);
  ASSERT_INT_EQ(OP_EXEC_FULL,        156);
  ASSERT_INT_EQ(OP_EXEC_STDIN,       157);
  ASSERT_INT_EQ(OP_EXEC_PIPE,        158);
  ASSERT_INT_EQ(OP_EXEC_BG,          159);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        160);
  ASSERT_INT_EQ(OP_SIGNAL,           161);
  ASSERT_INT_EQ(OP_CANCEL,           162);
  ASSERT_INT_EQ(OP_HALT,             163);
  TEST_PASS();
}
