...
static int test_opcode_enum(void) {
  ...
  ASSERT_INT_EQ(OP_CLOSE_LOOP,        136);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC,  137);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED, 138);
  ASSERT_INT_EQ(OP_DESTRUCTURE_VEC_REST, 139);
  ASSERT_INT_EQ(OP_DESTRUCTURE_NAMED_REST, 140);
  ASSERT_INT_EQ(OP_SPREAD,           141);
  ASSERT_INT_EQ(OP_CALL_SPREAD,      142);
  ASSERT_INT_EQ(OP_FOLD_SPREAD,      143);
  ASSERT_INT_EQ(OP_COLLECT_VARIADIC, 144);
  ASSERT_INT_EQ(OP_YIELD,            145);
  ASSERT_INT_EQ(OP_STREAM_NEXT,      146);
  ASSERT_INT_EQ(OP_COLLECT,          147);
  ASSERT_INT_EQ(OP_IS_STREAM_EXHAUSTED, 148);
  ASSERT_INT_EQ(OP_COUNT,            149);
  ASSERT_INT_EQ(OP_TAKE,             150);
  ASSERT_INT_EQ(OP_FIRST,            151);
  ASSERT_INT_EQ(OP_LINES,            152);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD,  153);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD,  154);
  ASSERT_INT_EQ(OP_GET_RESUME_POINT, 155);
  ASSERT_INT_EQ(OP_SET_RESUME_POINT, 156);
  ASSERT_INT_EQ(OP_YIELD_SM,        157);
  ASSERT_INT_EQ(OP_AWAIT_SM,        158);
  ASSERT_INT_EQ(OP_CALL_SUSPEND,   159);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_CELL, 160);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_CELL, 161);
  ASSERT_INT_EQ(OP_GET_STATE_FIELD_WIDE, 162);
  ASSERT_INT_EQ(OP_SET_STATE_FIELD_WIDE, 163);
  ASSERT_INT_EQ(OP_SYNTAX_SPLICE,   164);
  ASSERT_INT_EQ(OP_SYNTAX_OP,       165);
  ASSERT_INT_EQ(OP_INTERPRET,       166);
  ASSERT_INT_EQ(OP_INTERPRET_PRELUDE, 167);
  ASSERT_INT_EQ(OP_EXEC,             168);
  ASSERT_INT_EQ(OP_AWAIT_JOB,        169);
  ASSERT_INT_EQ(OP_SIGNAL,           170);
  ASSERT_INT_EQ(OP_HALT,             171);
  TEST_PASS();
}
