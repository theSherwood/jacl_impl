...
static int test_defmacro_registers_in_table(void) {
    ...
    ASSERT(cr.macro_table->count == 7);  /* 6 prelude (\, and, or, not, incr, timeout) + 1 user-defined */
...
static int test_defmacro_multiple_register(void) {
    ...
    ASSERT(cr.error_count == 0);
    ASSERT(cr.macro_table != NULL);
    ASSERT(cr.macro_table->count == 8);  /* 6 prelude (\, and, or, not, incr, timeout) + 2 user-defined */
